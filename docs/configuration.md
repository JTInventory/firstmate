...
FM_INACTIVE_OUTCOME_SECS=900       # minimum quiet seconds before a direct done/failed outcome is reconciled; clamps to 60..1800
FM_INACTIVE_OUTCOME_BUDGET_SECS=10 # total seconds allowed per home-local inactive-outcome scan; clamps to 1..300
FM_INACTIVE_OUTCOME_LOCK_WAIT_SECS=30 # seconds to wait for one child's spawn lock; clamps to 0..300
FM_INACTIVE_OUTCOME_STATE_TIMEOUT_SECS=10 # seconds allowed for one canonical child-state read; clamps to 1..300
FM_INACTIVE_OUTCOME_PARENT_STATUS_SCAN_SECS=1 # seconds allowed for parent status correlation checks; clamps to 1..60
FM_INACTIVE_OUTCOME_MAINTENANCE_RESERVE_SECS=1 # seconds reserved for receipt/route maintenance per scan; clamps to 1..60
FM_REPORTED_ROUTE_REPAIR_LIMIT=32    # maximum reported secondmate routes repaired per maintenance pass; clamps to 1..256
FM_PENDING_RECEIPT_REPUBLISH_LIMIT=32 # maximum pending receipts republished per maintenance pass; clamps to 1..256
FM_PANE_IDLE_INDEX_BUDGET_SECS=1     # seconds for one bounded pane-metadata index build; invalid/zero falls back to 1
FM_WAKE_QUEUE_STATUS_BUDGET_SECS=1   # seconds for one bounded wake-key status check; invalid/zero falls back to 1
FM_WATCH_CREW_STATE_TIMEOUT_SECS=1   # seconds for one bounded watcher current-state read; invalid/zero falls back to 1
FM_WATCH_SURFACE_REPAIR_BUDGET_SECS=1 # seconds for bounded watcher surface-retry repair; values above 60 clamp to 60
FM_WAKE_DRAIN_BATCH_ROWS=16           # maximum wake rows processed by one drain turn; invalid/zero falls back to 16
FM_WAKE_DRAIN_PRESENTATION_TIMEOUT_SECS=30 # seconds allowed to present one inactive receipt; invalid/zero falls back to 30 and values above 300 clamp to 300
