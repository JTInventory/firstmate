...
FM_CREW_STATE_NM_TIMEOUT=10   # total seconds shared by all no-mistakes queries in one fm-crew-state.sh invocation; nonnegative digits only, invalid values use 10
...
FM_SUPERVISION_PAUSE_RECONCILE_SECS=5   # total seconds shared across paused-worker fm-crew-state reconciliation in one supervision collection; nonnegative digits only, invalid values use 5
