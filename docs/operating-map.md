...
After the captain explicitly approves a PR merge, firstmate runs `FM_CAPTAIN_APPROVED_MERGE=1 FM_CAPTAIN_APPROVED_PR_HEAD=<presented sha> FM_CAPTAIN_APPROVED_PRESENTATION_NONCE=<presented nonce> bin/fm-pr-merge.sh <id> <full GitHub PR URL>` instead of calling `gh-axi pr merge` directly.
The wrapper derives the repository and PR number from the qualified GitHub URL, requires the approved nonce and current URL/head/base to equal the protected presentation immediately before the request, defaults to squash, and routes the merge through `gh-axi` with GitHub's atomic expected-head condition. GitHub exposes no expected-base merge condition, so the approved base is a preflight snapshot that can race with a simultaneous retarget or update. Immediate literal messages and expected-head leased remote branch deletion remain available; deletion runs non-interactively for at most 30 seconds and is warning-only after merge. Deferred auto-merge and bypass options are deliberately unsupported. Missing, stale, malformed, or unverifiable presentation evidence fails closed and requires fresh presentation and approval.
...
Teardown returns a scout worktree after its report exists, or returns a ship worktree only when work is landed and clean, subject to the [pooled-slot ownership gate](worker-isolation.md).
It refuses dirty or unlanded work unless the captain explicitly approves discard.
It retries only transient Git `index.lock` failures while returning a worktree or an ownership-gated leased secondmate home; any other return failure, or contested or unproven ownership, remains fail-closed.
