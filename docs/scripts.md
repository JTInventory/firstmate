...
| `fm-cbm-lib.sh`          | Optional codebase-memory-mcp (CBM) helpers: soft enable, cache/env exports, project allowlist, launch env prefix, brief orientation block, status line; never blocks callers when CBM is missing |
| `fm-cbm-index.sh`        | Host ops helper for CBM `status`, `list`, and `index` of allowlisted targets (`jt`, `firstmate`, `all`, or an absolute path); soft dependency, not required for fleet health |
