...
| Script                   | Description                                                                                                         |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| `fm-session-start.sh`    | Run the lock-first session-start flow: compose lock, bootstrap, and wake-drain output, then emit the READ-ONCE, compact FLEET STATE, CONTEXT, and next-step sections |
...
| `fm-herdr-session-cleanup.sh` | Conservatively retire stale restored-shell Herdr presentation children during a locked session start; home-local and non-fatal |
...
| `fm-line-cap-lib.sh`     | Shared per-line cap and ` [truncated]` marker for agent-facing digest output, sourced by session start              |
...
| `fm-inactive-reconcile.sh` | Scan this home's direct ordinary children on a bounded cadence, publish fingerprinted inactive terminal receipts, queue one replay wake, and let the locked wake drain present and acknowledge it |
