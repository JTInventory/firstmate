...
| `fm-fleet-sync.sh`       | Fetch clones, fast-forward safe default-branch states, self-heal clean detached ancestor drift, report unsafe drift as `STUCK:`, safely prune branches whose remote is gone, and accept a project name or `projects/<name>` for a deterministic single-clone sync |
...
| `fm-watch-arm.sh`        | Verified per-home watcher re-arm; reports `started`, `attached` (waits until that cycle ends), restart-only `healthy`, or `FAILED`; can reclaim a stale watcher lock whose stored PID identity no longer matches the live PID; `--restart` signals only this home's watcher and never attaches to a peer |
...
| `fm-supervision-model.sh` | Sourceable read-only model library used by `fm-supervise.sh` and future display consumers; invokes shared HOME-local tool path normalization, reconciles valid paused statuses within one bounded collection budget, and owns task classification ordering for PRs, scouts, and secondmates |
...
| `fm-send.sh`             | Send one verified literal line (or `--key Escape`) to a direct-report window; accepts only a recorded bare `fm-<id>` target or an explicit `session:window` target, refusing other bare names; exits non-zero on confirmed swallowed Enter; bare `kind=secondmate` targets are marked as from-firstmate; slash commands, codex `$...` skill invocations, and marked codex secondmate text get popup-settle before Enter; marked Codex secondmate text gets one delayed final Enter if generic retries leave it pending; text sends pause `FM_SEND_SETTLE` seconds after success |
