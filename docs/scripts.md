...
| `fm-supervise.sh`        | Read-only current-work checklist, plus `--json` / `--schema` for the shared `firstmate.supervision.v1` model; keeps completed scout reports on the report-teardown path and live secondmates on the persistent-direct-report path |
