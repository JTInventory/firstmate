...
| `fm-supervise.sh`        | Read-only current-work checklist, plus `--json` / `--schema` for the shared `firstmate.supervision.v1.1` model; exposes `backlog_consistency` drift findings for Radar; keeps scout reports with latest `done:` status on the report-teardown path and idle live secondmates on the persistent-direct-report path while still surfacing fresh secondmate follow-up statuses |
