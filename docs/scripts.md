...
| `fm-crew-state.sh`       | Print one stable current-state line for a crew by reconciling its matching no-mistakes run-step, including a green CI monitor awaiting captain merge, an explicit `paused:` external wait, and pane/status-log fallback even when the pane has closed |
