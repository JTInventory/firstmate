...
| `fm-no-mistakes-pr-target-guard.sh` | Fail closed before no-mistakes test/push/PR work if local git, direct push resolution, or the no-mistakes gate would target `kunchenguid/firstmate` instead of `JTInventory/firstmate`; allow upstream-owner `origin` fetch only with controlled-fork proof |
