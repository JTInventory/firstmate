...
| `fm-crew-state.sh`       | Print one stable current-state line for a crew by reconciling its matching no-mistakes run-step, including a green CI monitor awaiting captain merge, even when the pane has closed, with pane and status-log fallback |
