...
| `fm-supervise.sh`        | Read-only current-work checklist, plus `--json` / `--schema` for the shared `firstmate.supervision.v1` model; keeps scout reports with latest `done:` status on the report-teardown path and live secondmates on the persistent-direct-report path |
