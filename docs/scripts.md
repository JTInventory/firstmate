...
| `fm-watch-arm.sh`        | Verified per-home watcher re-arm; reports `started`, `healthy`, or `FAILED`; can reclaim a stale watcher lock whose stored PID identity no longer matches the live PID; `--restart` relaunches only this home's watcher |
