...
Fresh enrollment is first-lock-wins under the acquisition lock and requires live broker proof. It is allowed only for the operational checkout; a non-Git or cross-home path refuses. A live legacy update is accepted only when its exact PID is in the caller's ancestor chain and its Codex marker, when present, matches. Later hooks inherit the broker's protected live and durable descriptors, so authority is tied to the harness chain rather than a short-lived `SessionStart` hook. An authorized replacement rotates the live broker key while retaining the distinct durable receipt capability on FD 18. Only the production authority wrapper, after proving its own broker PID, start, executable identity, and script path, may create a detached durable custodian. The custodian repairs a changed record, but callers trust it only after validating the complete-record HMAC and a fresh challenge with the already-inherited durable capability. It never returns that capability to a process that did not inherit it. A replacement retaining FD 18 can rotate the live key and preserve migration journals and teardown receipts. A replacement retaining only FD 9, or neither descriptor, refuses because files, argv, ports, sockets, and same-UID processes are all replaceable and cannot provide an independent recovery root. A restarted secondmate without its own scoped durable descriptor must be enrolled again. A secondmate receives new scoped capabilities and never exports either primary descriptor to its agent. Missing, malformed, cross-session, worker-descended, or cryptographically invalid durable authority fails closed.
