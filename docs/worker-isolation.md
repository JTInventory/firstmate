...
An undeclared primary is accepted only through `bin/fm-session-authority-exec.sh` from the default-branch checkout. For first enrollment, the broker requires completely empty authority state and a live process chain whose session leader predates the checkout's Git common directory and contains no worker declaration. On macOS, the numeric leader PID comes from `getsid(2)` and must still be present in the caller's real ancestry; a leaderless or unreadable session refuses. A worker-created clean session cannot backdate that kernel process identity. For later enrollment it verifies the inherited key against live authority in its ancestor chain; interrupted recovery uses the same independent primary-only broker proof before transaction material is read. The broker remains the harness parent, holds the same immediately unlinked descriptor, and exports its exact PID, process start, and executable identity. `bin/fm-lock.sh` verifies that tuple, the broker script, ancestry, and shared descriptor identity. A direct caller with self-created hex has no broker proof. Crewmate launch closes the descriptor and clears the broker before executing the worker. An authorized home signs a one-use asymmetric ticket for a fresh secondmate without copying its live authority key. The short-lived signer must still hold the broker's exact authority descriptor. The private signing key exists only in shell memory and fresh anonymous pipes, with no filesystem pathname, bearer value in terminal command text, or sibling-readable environment. Spawn launches the randomly tagged wrapper through a backend operation that owns the new process: one serialized tmux exact-pane respawn-and-PID transaction or Herdr's direct agent-start response for the exact tab and pane. Assignment-prefixed launch environments, including the launcher's normalized `PATH`, are passed through `exec env`. It then verifies that response-derived process with tmux's exact pane PID or Herdr's authoritative pane process-info response, and both backends must keep their complete endpoint identity throughout enrollment. A later terminal injection can only send input to that process or replace it with a different PID, which invalidates enrollment. The ticket signs that exact PID, start value, and executable identity. On Linux, the broker, signer, and wrapper each use a real same-credential child open attempt to prove that procfs cannot expose any authority-bearing descriptor before creating its secret; readable or unrecognized isolation refuses startup. Darwin has no corresponding procfs descriptor path. The wrapper creates a separate anonymous consumer signing key in the already-launched process and sends only its public key in the consumption request. The signer binds that digest into its signed acceptance. After validating the acceptance against its own digest and signing the acknowledgment, the exact wrapper re-execs through validation into a confirmed stage carrying the acceptance digest and public signed ticket records. The signer requires that exact PID, start, executable identity, and confirmed-stage argument before accepting the acknowledgment, so replacing the request key cannot authorize a different process. The consumer private key never has a filesystem pathname, crosses an exec boundary, or leaves its anonymous descriptor. After acknowledgment, the parent rechecks the full endpoint identity and process before releasing lifecycle locks or reporting the spawn. Tmux metadata persists the exact pane and pane-scoped generation for every new task. Lifecycle operations and ordinary `fm-<id>` selectors share one strict parser that rejects duplicate or malformed pane data and proves the live pane still belongs to the recorded window and generation before transport. Legacy tmux metadata is migrated only when the live task already carries the complete secondmate declaration and exact cwd/process proof. A task launched before those declarations existed has no non-forgeable task evidence and therefore refuses migration. Upgrade such a live pre-port installation by stopping its secondmates, restarting the primary through `bin/fm-session-authority-exec.sh`, and spawning the secondmates again; do not reuse their old panes or metadata. A rolling launcher derives the verified broker script from the authority checkout when an older authority-aware live broker did not export the new script variable. The new pane applies its secondmate role and home before the authority wrapper consumes that ticket and creates a separate broker. Process names, argv text, a caller-created session, an arbitrary environment string, marker/cwd/pane evidence without declarations, and a bare `CODEX_THREAD_ID` never create or replace authority.

Fresh enrollment is first-lock-wins under the acquisition lock and requires the live broker proof. It is allowed only for the operational checkout; a non-Git or cross-home path refuses. A live legacy update is accepted only when its exact PID is in the caller's ancestor chain and its Codex marker, when present, matches. Later hooks inherit the broker and descriptor, so authority is tied to the durable harness chain rather than the short-lived `SessionStart` hook. A new broker may replace a dead authority after its recorded PID, start, and executable prove stale; a live or unreadable foreign owner refuses. Missing broker proof, inherited worker role, broken or newly created session ancestry, unreadable ancestry, malformed binding, invalid secondmate ticket, or conflicting live authority also refuses. Session-authority publication transactions retain a mode-600 recovery key and authenticate SHA-256 content digests, so recovery does not depend on the earlier launcher's key. Broker authorization occurs before the transaction key or manifest is read, and normal commit verifies the published lock, checkout binding, and authority digests before deleting recovery evidence. Tmux migration journals use a separate durable key for interruption recovery, while a binding over that key's SHA-256 digest and journal path is authenticated by the live primary authority descriptor. Replacing the journal-side key and journal cannot forge that binding. A replacement broker launched by the same primary inherits the descriptor and can recover; loss of the primary descriptor fails closed and requires the coordinated restart above.
