...
An undeclared primary is accepted only through `bin/fm-session-authority-exec.sh` from the default-branch checkout. For first enrollment, the broker requires completely empty authority state and a live process chain whose session leader predates the checkout's Git common directory and contains no worker declaration. On macOS, the numeric leader PID comes from `getsid(2)` and must still be present in the caller's real ancestry; a leaderless or unreadable session refuses. A worker-created clean session cannot backdate that kernel process identity. For later enrollment it verifies the inherited live key against authority in its ancestor chain, retains a separate durable receipt key, and rotates the live key before starting the replacement broker. The first compatible upgrade adopts the verified inherited live key as the durable receipt root before that rotation, so already-issued authenticated journals remain recoverable. Both keys stay on repeatable protected descriptors and the shell constructs HMAC-SHA256 from OpenSSL digest primitives, so keys never enter process arguments or readable storage. The broker remains the harness parent and exports its exact PID, process start, and executable identity. `bin/fm-lock.sh` verifies that tuple, the broker script, ancestry, and shared live descriptor identity. A direct caller with self-created hex has no broker proof. Crewmate launch closes both descriptors and clears the broker before executing the worker. An authorized home signs a one-use asymmetric ticket for a fresh secondmate. The confirmed wrapper closes every inherited primary descriptor and creates fresh live and durable capabilities scoped to the secondmate before exporting its broker environment. The short-lived signer must still hold the primary broker's exact live authority descriptor. The private enrollment signing key exists only in shell memory and fresh anonymous pipes, with no filesystem pathname, bearer value in terminal command text, or sibling-readable environment. Spawn launches the randomly tagged wrapper through a backend operation that owns the new process: one serialized tmux exact-pane respawn-and-PID transaction or Herdr's direct agent-start response for the exact tab and pane. Assignment-prefixed launch environments, including the launcher's normalized `PATH`, are passed through `exec env`. It then verifies that response-derived process with tmux's exact pane PID or Herdr's authoritative pane process-info response, and both backends must keep their complete endpoint identity throughout enrollment. A later terminal injection can only send input to that process or replace it with a different PID, which invalidates enrollment. The ticket signs that exact PID, start value, and executable identity. On Linux, the broker, signer, and wrapper each use a real same-credential child open attempt to prove that procfs cannot expose any authority-bearing descriptor before creating its secret; readable or unrecognized isolation refuses startup. Darwin has no corresponding procfs descriptor path. The wrapper creates a separate anonymous consumer signing key in the already-launched process and sends only its public key in the consumption request. The signer binds that digest into its signed acceptance. After validating the acceptance against its own digest and signing a stage-bound acknowledgment, the exact wrapper preserves the consumer private key only on protected FD 8 across the authorized and confirmed-stage re-execs. The confirmed stage publishes a separately signed `stage=final` receipt, closes FD 8, and only then becomes authorized; an acknowledgment cannot be replayed as final acceptance. The signer validates that final receipt before success. The parent rechecks the full endpoint identity and process before releasing lifecycle locks or reporting the spawn. Tmux metadata persists the exact pane and pane-scoped generation for every new task. Lifecycle operations and ordinary `fm-<id>` selectors share one strict parser that rejects duplicate or malformed pane data and proves the live pane still belongs to the recorded window and generation before transport. Legacy tmux metadata is migrated only when the live task already carries the complete secondmate declaration, exact cwd/process proof, and a durable primary-authenticated launch receipt. A task launched before those receipts existed has no non-forgeable task evidence and therefore refuses migration. Upgrade such a live pre-port installation by stopping its secondmates, restarting the primary through `bin/fm-session-authority-exec.sh`, and spawning the secondmates again; do not reuse their old panes or metadata. A rolling launcher derives the verified broker script from the authority checkout when an older authority-aware live broker did not export the new script variable. Process names, argv text, a caller-created session, an arbitrary environment string, marker/cwd/pane evidence without an authenticated receipt, and a bare `CODEX_THREAD_ID` never create or replace authority.

Fresh enrollment is first-lock-wins under the acquisition lock and requires live broker proof. It is allowed only for the operational checkout; a non-Git or cross-home path refuses. A live legacy update is accepted only when its exact PID is in the caller's ancestor chain and its Codex marker, when present, matches. Later hooks inherit the broker's protected live and durable descriptors, so authority is tied to the harness chain rather than a short-lived `SessionStart` hook. An authorized replacement rotates the live broker key while retaining the distinct durable receipt capability. A replacement that retains only the live descriptor must recover an existing custodian root before using the compatibility adoption path. Only the production authority wrapper, after proving its own broker PID, start, executable identity, and script path, may create or replace a detached durable custodian; sourcing the library with caller-created descriptors cannot authorize that operation. Before the custodian can publish its record, that wrapper gives its exact PID and session identity a launch certificate authenticated by both authority roots and a one-use signing channel on reserved FD 17 whose public half is pinned in the custodian's immutable arguments. FD 17 must be unused and sibling-isolated before key creation. At startup the custodian independently proves that its actual parent is that exact production wrapper and that its inherited FDs 9, 18, and 17 are the same protected channels still held by the parent. The parent retains FD 17 only through this startup proof and closes it before launching the harness command, so a later broker child cannot invoke the public custodian script with self-created keys. The custodian validates the certificate, closes the live, durable, and signing descriptors, clears all live-broker state, and keeps the durable receipt root and signing private key only in shell memory outside the broker and harness process tree. A broker that still holds the durable root validates the custodian record with a root-bound challenge and can retire a stale record into a replacement carrying the same root. After complete broker-tree loss, a newly authorized wrapper publishes a one-use RSA recovery public key in its immutable arguments; the custodian revalidates the exact wrapper, original kernel session and start identity, process start and identity, and permitted primary or exact-home secondmate ancestry before returning the receipt root encrypted to that key. Its response binds the ciphertext, custodian, requester, and public key with the durable root. Before installing the candidate root on FD 18, the wrapper sends a second fresh challenge to the same live custodian and requires both a candidate-root HMAC and a signature matching the public key pinned in that exact process's arguments. Only then does it install the root, validate the complete record again, and rotate the live key. A restarted secondmate must complete this recovery before it can create any new scoped key, so replacement cannot discard earlier journals or receipts. Migration journals, launch receipts, and teardown post-action receipts authenticate their complete bodies with that capability, so interrupted recovery survives both live-key rotation and authorized process-tree replacement while same-UID workers cannot recompute a receipt. A secondmate receives new scoped capabilities and never exports either primary descriptor to its agent. Missing, stale without an authenticated retaining broker, malformed, cross-session, worker-descended, or cryptographically invalid custodian recovery fails closed.
