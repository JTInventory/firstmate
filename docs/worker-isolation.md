...
FM-027 is intentionally Linux-only. Unsupported systems and architectures exit 125 before authority tools are checked. macOS process inspection is diagnostic only; it does not provide secondmate containment or session authority.
...
An undeclared primary is accepted only through `bin/fm-session-authority-exec.sh` from the default-branch checkout. For first enrollment, the broker requires completely empty authority state and a live process chain whose session leader predates the checkout's Git common directory and contains no worker declaration. FM-027 is Linux-only; unsupported systems and architectures exit 125 before authority tools are checked, and Darwin has no secondmate authority path. A worker-created clean session cannot backdate that kernel process identity. For later enrollment it verifies the inherited live key against authority in its ancestor chain, retains a separate durable receipt key, and rotates the live key before starting the replacement broker. The first compatible upgrade adopts the verified inherited live key as the durable receipt root before that rotation, so already-issued authenticated journals remain recoverable. Primary authority keys stay on repeatable protected descriptors and the shell constructs HMAC-SHA256 from OpenSSL digest primitives, so keys never enter process arguments or readable storage. Concurrent cold starts first serialize on the state-scoped provisioning gate; only the winner may generate the first roots or publish the broker. On Linux systems where procfs makes those descriptors reopenable, a secondmate instead uses `bin/fm-session-authority-broker.py`: a private per-home `AF_UNIX` socket whose name is only a locator. The server obtains the caller PID from `SO_PEERCRED`, verifies the real client script and cwd, requires the exact secondmate task and owner home in its process ancestry, refuses any ancestry containing a crewmate declaration, and completes a fresh challenge-response using the inherited capability. Neither the socket name nor the broker record authorizes a lease. Its private record contains only process and socket identity, never a key or HMAC. The primary broker remains the harness parent and exports its exact PID, process start, and executable identity. `bin/fm-lock.sh` verifies that tuple, the broker script, ancestry, and authority channel. A direct caller with self-created hex has no broker proof. Crewmate launch closes both descriptors and clears the broker before executing the worker. An authorized home signs a one-use asymmetric ticket for a fresh secondmate. The confirmed wrapper closes every inherited primary descriptor and creates fresh authority scoped to the secondmate before exporting its broker environment. The short-lived signer must still hold the primary broker's exact live authority descriptor. The private enrollment signing key exists only in shell memory and fresh anonymous pipes, with no filesystem pathname, bearer value in terminal command text, or sibling-readable environment. Spawn launches the randomly tagged wrapper through a backend operation that owns the new process: one serialized tmux exact-pane respawn-and-PID transaction or Herdr's direct agent-start response for the exact tab and pane. Assignment-prefixed launch environments, including the launcher's normalized `PATH`, are passed through a shell-wrapped `exec sh -c`. It then verifies that response-derived process with tmux's exact pane PID or Herdr's authoritative pane process-info response, and both backends must keep their complete endpoint identity throughout enrollment. A later terminal injection can only send input to that process or replace it with a different PID, which invalidates enrollment. The ticket signs that exact PID, start value, and executable identity. On Linux, every remaining authority-bearing descriptor still uses a real same-credential child open attempt to prove that procfs cannot expose it; readable or unrecognized isolation refuses that descriptor path. Darwin authority descriptors are unsupported. The wrapper creates a separate anonymous consumer signing key in the already-launched process and sends only its public key in the consumption request. The signer binds that digest into its signed acceptance. After validating the acceptance against its own digest and signing a stage-bound acknowledgment, the exact wrapper preserves the consumer private key only on protected FD 8 across the authorized and confirmed-stage re-execs. The confirmed stage publishes a separately signed `stage=final` receipt, closes FD 8, and only then becomes authorized; an acknowledgment cannot be replayed as final acceptance. The signer validates that final receipt before success. The parent rechecks the full endpoint identity and process before releasing lifecycle locks or reporting the spawn. Tmux metadata persists the exact pane and pane-scoped generation for every new task. Lifecycle operations and ordinary `fm-<id>` selectors share one strict parser that rejects duplicate or malformed pane data and proves the live pane still belongs to the recorded window and generation before transport. Legacy tmux metadata is migrated only when the live task already carries the complete secondmate declaration, exact cwd/process proof, and a durable primary-authenticated launch receipt. A task launched before those receipts existed has no non-forgeable task evidence and therefore refuses migration. Upgrade such a live pre-port installation by stopping its secondmates, restarting the primary through `bin/fm-session-authority-exec.sh`, and spawning the secondmates again; do not reuse their old panes or metadata. A rolling launcher derives the verified broker script from the authority checkout when an older authority-aware live broker did not export the new script variable. Process names, argv text, a caller-created session, an arbitrary environment string, marker/cwd/pane evidence without an authenticated receipt, and a bare `CODEX_THREAD_ID` never create or replace authority.

Fresh enrollment is first-lock-wins under the acquisition lock and requires live broker proof. It is allowed only for the operational checkout; a non-Git or cross-home path refuses. A live legacy update is accepted only when its exact PID is in the caller's ancestor chain and its Codex marker, when present, matches. Later hooks inherit the broker's protected live and durable descriptors, so authority is tied to the harness chain rather than a short-lived `SessionStart` hook. An authorized replacement rotates the live broker key while retaining the distinct durable receipt capability on FD 18. Only the production authority wrapper, after proving its own broker PID, start, executable identity, and script path, may create a detached durable custodian. The custodian repairs a changed record. A caller retaining FD 18 must authenticate that candidate root against the existing custodian with a fresh root HMAC and custodian signature before replacement, and the complete validation-and-replacement transaction is serialized by a state-scoped lock. After broker descriptor loss, an authorized wrapper creates a one-use RSA recovery key. The custodian verifies the exact wrapper, original session identity, live process identity, and permitted primary or same-home secondmate ancestry before returning the root encrypted to that key. The response binds the ciphertext and both endpoint identities with the durable root, and the wrapper proves the recovered root against a second signed challenge before installing FD 18. A same-home secondmate that already retains valid FD 18 keeps it while rotating only its live key. A secondmate receives new scoped capabilities and never exports either primary descriptor to its agent. Missing, malformed, cross-session, worker-descended, unrelated-root, or cryptographically invalid durable authority fails closed.
...
The declaration marker matters here beyond home isolation: it is the provider-independent identity key, and it is the only source that survives a restore which re-parents or relabels panes. Linux reads it from procfs. macOS process inspection is diagnostic only and cannot establish FM-027 authority.
...
On macOS it may enumerate processes with `ps` and read each candidate through `kern.procargs2` for diagnostics.
...
Before an unpublished lease return, spawn stages an exact task, home, and lease-holder claim and invokes `treehouse return --force` with `--if-lease-holder`. A failed or unknown provider result restores that exact staged claim; if the committed transition cannot be recorded, it writes the same unknown-lease diagnostic and keeps reuse blocked.
