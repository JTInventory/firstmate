...
An undeclared primary is accepted only from the default-branch checkout with an anonymous descriptor supplied by the trusted session launcher. `bin/fm-session-authority-exec.sh` cannot enroll itself: it first verifies the inherited key against a live authority in its ancestor chain, then delegates a new immediately unlinked descriptor. Crewmate launch closes that descriptor before executing the worker, while secondmate launch delegates a separate descriptor before applying the secondmate declaration. `bin/fm-lock.sh` stores only an HMAC-SHA256 proof bound to the exact home, checkout, owner, process start, and executable identity. Process names, argv text, an arbitrary environment string, and a bare `CODEX_THREAD_ID` never create or replace authority.

Fresh enrollment is first-lock-wins under the acquisition lock and requires the trusted launcher's external descriptor. It is allowed only for the operational checkout itself or a Git worktree with the same canonical common directory; a non-Git cross-home path refuses. A live legacy update is accepted only when its exact PID is in the caller's ancestor chain and its Codex marker, when present, matches. Later hooks transfer the inherited descriptor, so authority is tied to the durable harness chain rather than the short-lived `SessionStart` hook. A new descriptor may replace a dead authority after its recorded PID, start, and executable prove stale; a live or unreadable foreign owner refuses. Missing capability, inherited worker role, unreadable ancestry, malformed binding, or conflicting live authority also refuses. Session-authority publication transactions retain a mode-600 recovery key and authenticate SHA-256 content digests, so recovery does not depend on the launcher's later lifetime.
