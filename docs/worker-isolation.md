...
An undeclared primary is accepted only from the default-branch checkout with a valid `FM_SESSION_AUTHORITY_CAPABILITY`. The primary launcher must create at least 32 random characters, export the capability before starting the harness, and keep it out of every worker environment. `bin/fm-lock.sh` stores only an HMAC-SHA256 proof bound to the exact home, checkout, owner, process start, and executable identity. Process names, argv text, and a bare `CODEX_THREAD_ID` never create or replace authority.

Fresh enrollment is first-lock-wins under the acquisition lock. It is allowed only for the operational checkout itself or a Git worktree with the same canonical common directory; a non-Git cross-home path refuses. A live legacy lock without an authority record migrates only when its exact PID is in the caller's ancestor chain and its Codex marker, when present, matches. Later hooks and PID-isolated commands transfer authority by presenting the same capability, so authority is not tied to the short-lived `SessionStart` hook. Missing or invalid capability, inherited worker role, unreadable ancestry, foreign legacy owner, malformed binding, or conflicting keyed record refuses.
