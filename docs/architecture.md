...
When `HERDR_ENV=1` is present without explicit configuration, Herdr is auto-detected; `$TMUX` wins when nested. Herdr tasks use one workspace per firstmate home and one tab per task, while treehouse remains the worktree provider. The AFK daemon supports Herdr supervisor target resolution and injection when explicitly selected. On capable Herdr protocol-16 sessions, the watcher can also use the native `pane.agent_status_changed` stream as an optional fast path; capability, socket, subscription, and reader failures fall back to polling. The detailed Herdr lifecycle and safety contract lives in [herdr-backend.md](herdr-backend.md).
