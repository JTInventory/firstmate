...
Routine re-arms go through `bin/fm-watch-arm.sh`, which forks the watcher as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, and prints exactly one honest status line (`started` / `healthy` / `FAILED`, the last exiting non-zero) - never a false `already running` off a dying process or reused PID.
The shared lock helper preserves live-held behavior for locks with no process identity, but watcher locks that stored a stale `pid-identity` can be reclaimed once the current live PID no longer matches that identity.
...
The read-only supervision model first adds existing `$HOME/.nvm/versions/node/*/bin` and `$HOME/.local/bin` directories to `PATH`, which lets SSH and other non-interactive sessions find HOME-installed Axi tools before deciding whether GitHub proof is available.
