...
Grok-specific follower ownership, status-to-badge mapping, and `fm-watch-session` refusal/override rules are owned by the [Grok supervision protocol](supervision-protocols/grok.md).
...
For ship and scout work, `fm-spawn.sh` acquires a task-bound durable lease with `treehouse get --lease --lease-holder <task>`, enters the returned path, and then refuses to launch unless it resolves to a real git worktree root of the target project: its physical git common dir must match the target project and its HEAD must exist in that repo. The durable lease survives an exited or paused worker and is released only by Firstmate's ownership-gated teardown, so Treehouse cannot reissue a stamped slot between the worker exit and teardown. For a live endpoint, that gate proves occupancy from the exact endpoint-bound process rather than an unrelated whole-host process census; missing endpoint PID, cwd, or declaration proof retains the lease. The path must also differ from the active operational home and the Firstmate root. A different git root is not an acceptable isolation result; spawn refuses publication and uses the backend's exact endpoint-retirement path, retaining ownership evidence when retirement cannot be proven.
...
If tmux does not return a valid ID, or if its complete window, pane, and generation identity cannot be proven, spawn aborts before it sends a pane command and retains ownership evidence. Once that identity is bound, setup cleanup uses the immutable endpoint and retires it only after the backend reports it dead. Herdr applies the same fail-closed rule to the complete session, workspace, tab, pane, and generation identity.
...
That spawn-time check is not durable by itself. A restored session can resume an agent in the source checkout, a child can inherit the launching home's environment, and pooled slots can be recorded by more than one task. [Worker isolation](worker-isolation.md) owns the full four-mechanism contract: launch-time home declarations, process-cwd proof, pooled-slot ownership gates, and the session-start `ISOLATION:` sweep.

The callable `bin/fm-turnend-guard.sh` is the script-only "no turn ends blind" backstop for a primary firstmate checkout and a genuinely marked secondmate home. It blocks with exit 2 only when child metadata is in flight and this home's watcher cannot be proved live by matching PID identity, home/path lock fields, and a fresh beacon. Linked child crew/scout worktrees are exempt because their git-dir differs from git-common-dir and they do not carry the secondmate marker; declared task workers are always excluded even if they inherit primary root/state values. JT deliberately does not install live PreToolUse or harness hooks for this guard in Phase B; callers must invoke it explicitly.
...
Slot release is separately fail-closed on ownership evidence. A pooled slot another task still holds is retained, and `--force` does not waive that gate; [worker isolation](worker-isolation.md) owns the release and reclaim rules.
