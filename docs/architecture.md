...
If tmux does not return a valid ID or cannot retain the canonical title, spawn attempts cleanup of the uniquely identified newly created window and aborts before it sends a pane command; if cleanup is not proven, ownership evidence is retained. Herdr applies the same fail-closed rule when it cannot return or retire a task tab/pane target.
