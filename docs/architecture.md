...
The wrapper accepts only a qualified GitHub PR URL, requires the approved presentation nonce and current forge URL/head/base to match the receipt immediately before the request, defaults to squash, and routes the merge through `gh-axi` with GitHub's atomic expected-head condition. The API has no expected-base condition, so the base remains a preflight snapshot and can race with a simultaneous retarget or update. Immediate literal commit subject/body and expected-head leased remote branch deletion remain available; deferred auto-merge and bypass options are deliberately unsupported. A missing, malformed, stale, or unverifiable receipt fails closed and requires a fresh presentation and approval.
