...
It also classifies a completed scout report as teardown work before PR or missing-worktree checks, and a live `kind=secondmate` record as a persistent direct report unless the latest status is `done:`, `blocked:`, `needs-decision:`, or `failed:`.
