...
For ship and scout work, `fm-spawn.sh` waits for `treehouse get` and then refuses to launch unless the task resolves to a real git worktree root of the target project: its physical git common dir must match the target project and its HEAD must exist in that repo. The path must also differ from the active operational home and the Firstmate root. A different git root is not an acceptable isolation result; the fresh window is killed and no task meta is recorded on refusal.
