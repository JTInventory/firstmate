...
For ship and scout work, `fm-spawn.sh` acquires a task-bound durable lease with `treehouse get --lease --lease-holder <task>`, enters the returned path, and then refuses to launch unless it resolves to a real git worktree root of the target project: its physical git common dir must match the target project and its HEAD must exist in that repo. The durable lease survives an exited or paused worker and is released only by Firstmate's ownership-gated teardown, so Treehouse cannot reissue a stamped slot between the worker exit and teardown. The path must also differ from the active operational home and the Firstmate root. A different git root is not an acceptable isolation result; the fresh window is killed and no task meta is recorded on refusal.
