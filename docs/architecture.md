...
The wrapper accepts only a qualified GitHub PR URL, requires the approved presentation nonce and current forge URL/head/base to match the receipt, defaults to squash, and supplies that SHA to GitHub's atomic merge API. Immediate literal commit subject/body and safely encoded remote branch deletion options remain available; deferred auto-merge and bypass options are deliberately unsupported. A missing, malformed, stale, or unverifiable receipt fails closed and requires a fresh presentation and approval.
