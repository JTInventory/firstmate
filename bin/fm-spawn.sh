...
#   --display-title supplies the deterministic Herdr task phrase.
...
spawn_abort_cleanup() {
  local status=$? endpoint_cleanup_status=1 slot_returned=0
...
spawn_existing_meta_allows_retry() {  # <meta>
  local meta=$1 old_backend old_target old_window_target old_session old_pane old_state old_slot_state
  local old_endpoint_recovery old_window_id old_window_name old_window_presence
  ...
  if [ "$old_endpoint_recovery" = 1 ] && [ "$old_backend" = tmux ]; then
    old_window_target=$old_target
    case "$old_window_target" in
      *:*) old_session=${old_window_target%%:*} ;;
      *)
        echo "error: existing task $ID has an invalid endpoint recovery target; refusing duplicate launch" >&2
        return 1
        ;;
    esac
    ...
    fm_backend_source tmux || return 1
    if fm_backend_tmux_window_presence "$old_session" "$old_target"; then
      old_window_name=$(fm_backend_task_name tmux "$old_target" 2>/dev/null) || {
        echo "error: existing endpoint recovery window for $ID could not be read; refusing duplicate launch" >&2
        return 1
      }
    ...
    case "$old_window_presence" in
      1) return 0 ;;
      2)
        echo "error: could not establish whether the endpoint recovery window for $ID still exists; refusing duplicate launch" >&2
        return 1
        ;;
      *) return 1 ;;
    esac
...
if [ -e "$STATE/$ID.meta" ] || [ -L "$STATE/$ID.meta" ]; then
  ...
  spawn_existing_meta_allows_retry "$STATE/$ID.meta" || exit 1
  SPAWN_RECOVERY_META_REPLACE_ALLOWED=1
fi
DISPLAY_LABEL=
TASK_KEY=
...
EOF
    ...
    # (the second field empty when this call adopts a pre-existing workspace
    # rather than creating a fresh one). Split on the guaranteed single tab
    # character; the seeded tab id is threaded through to create_task
    # untouched, which is the only function permitted to prune it.
