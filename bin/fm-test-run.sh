...
family_for_basename() {
  case "$1" in
    ...
    fm-concurrent-primary-identity.test.sh|fm-cross-home-lifecycle-scan.test.sh|\
    fm-update.test.sh|fm-worker-isolation.test.sh|fm-session-authority-broker.test.sh|\
    ...
    fm-afk-inject-e2e.test.sh|fm-afk-return.test.sh)
      printf '%s\n' afk
      ;;
    fm-bearings-snapshot.test.sh|fm-fleet-snapshot.test.sh)
      printf '%s\n' snapshot-bearings
      ;;
