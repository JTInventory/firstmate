...
# UTF-8 text rather than a terminal control byte. The ASCII 0x1f separator is
# terminal-unsafe: some terminal-backed composers interpret it as a control
# action and erase the visible marker before the agent receives the request.
...
# message. printf -v is intentional: command substitution would strip trailing
# newline bytes from a routed request.
fm_message_mark_from_firstmate() {  # <message> <result-var>
  ...
  if fm_message_from_firstmate "$message"; then
    ...
    printf -v "$result_var" '%s' "${FM_FROMFIRST_MARK}${message}"
  fi
}
