...
4. Run \`git fetch "\$PUSH_ENDPOINT" "+\$BASE_REF:refs/remotes/origin/\$DEFAULT"\`.
...
6. Run \`git fetch "\$PUSH_ENDPOINT" "+\$FEATURE_REF:refs/remotes/origin/fm/$ID"\`, snapshot \`EXPECTED=\$(git rev-parse refs/remotes/origin/fm/$ID)\`, and require \`git merge-base --is-ancestor "\$EXPECTED" HEAD\`; if the ancestry check fails, append \`blocked: remote feature branch diverged; refusing to overwrite remote-only commits\` and stop.
