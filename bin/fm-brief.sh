...
8. Rebase onto the immutable \`DEFAULT_OID\` with \`git rebase "\$DEFAULT_OID"\` and resolve ordinary conflicts. Set \`POST_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed after rebase\` and stop. Atomically rewrite all eleven bound fields through \`LEASE_CHECKPOINT_TMP\`, preserving \`repo=\$REPO_ID\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, and \`pre_head=\$PRE_HEAD\`, while setting \`post_head=\$POST_HEAD\`, \`phase=ready-to-push\`, and \`attempt=0\`. If this second write or rename fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, append \`blocked: direct-PR ready checkpoint write failed; recover from rebase-in-progress state\`, and stop without pushing.
9. Before publication, require \`WORKFLOW=post-conflict\`, \`PHASE=ready-to-push\`, \`ATTEMPT\` equal to 0, 1, or 2, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`; otherwise append \`blocked: direct-PR publication state changed; refusing push\` and stop. The checkpoint attempt is the total authorized push count across every invocation. Every attempt or exhaustion transition must atomically preserve \`repo=\$REPO_ID\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, \`pre_head=\$PRE_HEAD\`, and \`post_head=\$POST_HEAD\`, change only the defined \`phase\` and \`attempt\`, write all eleven validated fields with mode 0600 through stable \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. On any such write or rename failure, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, retain the primary checkpoint, append \`blocked: direct-PR publication checkpoint transition failed\`, and stop without pushing. Classify \`git ls-remote --exit-code origin "\$FEATURE_REF"\` as exit 0 with its OID, exit 2 with no ref, or any other status as \`blocked: remote feature retry lookup failed\` and stop. Exit 0 at \`POST_HEAD\` proves publication already succeeded; exit 0 at \`EXPECTED\` means unchanged; a different OID or exit 2 means movement. On movement, remove the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\`, then restart safe validation. For \`ATTEMPT=0\`, atomically rewrite the validated checkpoint with \`attempt=1\` before the first \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`; a transition failure uses that cleanup and blocker without pushing. If that push fails or recovery starts with \`ATTEMPT=1\`, classify the remote first: published proceeds to the published-state transition, movement restarts, and unchanged atomically rewrites \`attempt=2\` before the second and final identical push. If that transition fails, use that cleanup and blocker without pushing. If the second push fails or recovery starts with \`ATTEMPT=2\`, classify the remote first: published proceeds to the published-state transition, movement restarts, and unchanged atomically rewrites \`phase=push-exhausted\` with \`attempt=2\`, retains the checkpoint, appends \`blocked: direct-PR publication retry exhausted; checkpoint retained\`, and stops without another push. Recovery from \`PHASE=push-exhausted\` performs no remote lookup or automatic push and retains that blocker. After either push succeeds or remote classification proves \`POST_HEAD\` is published, atomically preserve \`repo=\$REPO_ID\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, \`pre_head=\$PRE_HEAD\`, \`post_head=\$POST_HEAD\`, and \`attempt=\$ATTEMPT\`, set \`phase=published-awaiting-pr\`, write all eleven fields with mode 0600 through stable \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. If that write or rename fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, retain the durable prior checkpoint, append \`blocked: direct-PR published checkpoint transition failed; prior checkpoint retained\`, and stop before any PR action. Then enter step 10.
10. PR reconciliation is a separate \`PHASE=published-awaiting-pr\` path. Require \`WORKFLOW=post-conflict\`, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`. Immediately revalidate \`git ls-remote --exit-code origin "\$FEATURE_REF"\`: exit 0 must return exactly \`POST_HEAD\`; exit 2 must append \`blocked: published direct-PR feature ref deleted; checkpoint retained\`; any other status must append \`blocked: published direct-PR feature lookup failed; checkpoint retained\`; and a different OID must append \`blocked: published direct-PR feature moved; checkpoint retained\`. Stop on every blocker without cleanup. In this exact repository, list PRs in all states for head \`BRANCH\` and base \`DEFAULT\`, and require every match to have that exact repository, head, and base identity. Require exactly one match. No match, multiple matches, or lookup failure must append \`blocked: direct-PR PR identity reconciliation failed; checkpoint retained\` and stop. If the sole match is closed or merged, append \`blocked: direct-PR PR is {url} ({state}); checkpoint retained\` and stop without creating a replacement. Require the sole open PR head OID to equal \`POST_HEAD\`; otherwise append \`blocked: direct-PR PR head moved; checkpoint retained\` and stop. Rerun \`fm-pr-check\`, then remove the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\`; only then is reconciliation complete.
