...
Receipt and private-ref safety for this workflow: extend every checkpoint write and parse to exactly sixteen fields by adding \`pr_url=\`; hydrate \`PR_URL\` only as validated data and preserve it on every transition. \`published-awaiting-pr\` requires empty \`PR_URL\`; \`pr-check-confirmed\` requires canonical \`PR_URL\` bound to the existing exact \`POST_HEAD\` and full checkpoint identity. Every base or feature private-ref fetch must exit zero and its fetched OID must equal the immediately preceding live lookup OID before any \`rev-parse\`, rebase, lease, or ancestry use; otherwise append \`blocked: direct-PR private fetch failed or mismatched live lookup; private refs retained\` and stop. Final success must validate that \`PRIVATE_REF_ROOT\` contains only \`BASE_FETCH_REF\` and optional \`FEATURE_FETCH_REF\`, delete only those exact refs with \`git update-ref -d\`, then remove checkpoint state; ambiguity blocks cleanup.
...
Firstmate receipt ownership: Before that call Firstmate must parse and validate the full sixteen-field checkpoint, re-list the exact PR repository/head/base identity, and require its head OID equals \`POST_HEAD\`. Only after the guarded check exits zero, Firstmate atomically preserves the full checkpoint identity, sets \`pr_url={canonical url}\` and \`phase=pr-check-confirmed\`, writes all sixteen fields through \`LEASE_CHECKPOINT_TMP\` with mode 0600, and renames it to \`LEASE_CHECKPOINT\`; any validation, check, write, or rename failure retains the prior checkpoint and blocks. An existing exact \`pr-check-confirmed\` receipt is idempotent proof and prevents another check or metadata/poll publication after interruption. Worker finalization must require that exact receipt, repeat all live endpoint, default, feature, and PR identity checks including head OID equality, validate the private namespace contains only \`BASE_FETCH_REF\` and \`FEATURE_FETCH_REF\`, delete those exact refs with \`git update-ref -d\`, remove only the validated checkpoint files, and emit the single terminal \`done\`.
