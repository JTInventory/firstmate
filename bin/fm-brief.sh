...
Firstmate-owned phase transitions: before invoking \`bin/fm-pr-check.sh "\$TASK_ID" "{canonical url}"\`, Firstmate must parse the primary sixteen-field checkpoint, revalidate the exact repository/head/base identity and require the PR head OID equals \`POST_HEAD\`, then atomically preserve every field, set canonical \`pr_url={canonical url}\` and \`phase=pr-check-pending\`, write mode 0600 through \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`; failure retains the prior checkpoint and blocks before the check. In \`pr-check-pending\`, Firstmate must validate the canonical task metadata plus \`state/<task>.check.sh\`, \`state/<task>.pr-poll\`, and \`state/<task>.pr-poll-registration\` with \`fm_pr_poll_artifacts_valid\` against the same task and canonical PR URL, and require metadata \`pr_head=POST_HEAD\`. If all exact artifacts already exist, atomically advance to \`pr-check-confirmed\` without rerunning the check. If task metadata contains neither \`pr=\` nor \`pr_head=\` and all three poll artifacts are provably absent, run \`fm-pr-check\` exactly once; after exit zero, revalidate the complete exact artifact set and metadata head, then atomically advance to \`pr-check-confirmed\`. Any partial, mismatched, ambiguous, failed, or interrupted publication retains \`pr-check-pending\`, appends \`blocked: direct-PR PR-check artifact reconciliation failed; checkpoint retained\`, and never reruns automatically. Both phase transitions preserve all sixteen fields, the canonical \`PR_URL\`, exact \`POST_HEAD\`, mode 0600, stable-temp atomic rename, and prior-checkpoint retention on failure. In \`pr-check-confirmed\`, the worker must require that exact receipt, repeat all live endpoint, default, feature, and PR identity checks including head OID equality, validate the private namespace contains only \`BASE_FETCH_REF\` and \`FEATURE_FETCH_REF\`, delete both exact refs in one \`git update-ref --stdin\` transaction, remove only the validated checkpoint files, and emit the single terminal \`done\`; any failure retains state and blocks without publishing metadata or a poll again.
