...
6. Keep \`EXPECTED\` unchanged across the rebase and transient push retries. Push with \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "HEAD:\$FEATURE_REF"\`. If the push fails, query the remote feature ref without overwriting \`EXPECTED\`: when its OID still equals \`EXPECTED\` (including confirmed absence when \`EXPECTED\` is empty), retry that same lease push; restart this workflow and ancestry validation only after a lease rejection or confirmed remote change; if the retry lookup fails, append \`blocked: remote feature retry lookup failed\` and stop. Open the PR with \`gh-axi\` only after the push succeeds.
...
5. Rebase onto \`origin/\$DEFAULT\` and resolve ordinary conflicts.
6. Keep \`EXPECTED\` unchanged across the rebase and transient push retries. Update the open PR with \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "HEAD:\$FEATURE_REF"\`. If the push fails, query the remote feature ref without overwriting \`EXPECTED\`: when its OID still equals \`EXPECTED\`, retry that same lease push; restart this workflow and ancestry validation only after a lease rejection or confirmed remote change; if the retry lookup fails, append \`blocked: remote feature retry lookup failed\` and stop. Reconciliation is complete only after the push succeeds.
