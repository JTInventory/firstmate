...
4. Derive the task-scoped private refs \`PRIVATE_REF_ROOT=refs/firstmate/direct-pr/\$TASK_ID\`, \`BASE_FETCH_REF=\$PRIVATE_REF_ROOT/base\`, and \`FEATURE_FETCH_REF=\$PRIVATE_REF_ROOT/feature\`; their destination namespace is bound by the validated checkpoint to this task, repository, base, and feature identity and must not be shared with another task. Run \`git fetch "\$PUSH_ENDPOINT" "+\$BASE_REF:\$BASE_FETCH_REF"\`.
5. Run \`git ls-remote --exit-code "\$PUSH_ENDPOINT" "\$FEATURE_REF"\` and require exit 0; exit 2 means the published feature ref is missing, and any other status is a lookup failure, so append \`blocked: published remote feature lookup failed\` and stop for either case.
6. Run \`git fetch "\$PUSH_ENDPOINT" "+\$FEATURE_REF:\$FEATURE_FETCH_REF"\`, snapshot \`EXPECTED=\$(git rev-parse "\$FEATURE_FETCH_REF")\`, and require \`git merge-base --is-ancestor "\$EXPECTED" HEAD\`; if the ancestry check fails, append \`blocked: remote feature branch diverged; refusing to overwrite remote-only commits\` and stop.
...
10. PR reconciliation is a separate \`PHASE=published-awaiting-pr\` path. Require \`WORKFLOW=post-conflict\`, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`. Immediately repeat the step 1 exact endpoint and live symref revalidation, then revalidate \`git ls-remote --exit-code "\$PUSH_ENDPOINT" "\$FEATURE_REF"\`: exit 0 must return exactly \`POST_HEAD\`; exit 2 must append \`blocked: published direct-PR feature ref deleted; checkpoint retained\`; any other status must append \`blocked: published direct-PR feature lookup failed; checkpoint retained\`; and a different OID must append \`blocked: published direct-PR feature moved; checkpoint retained\`. Stop on every blocker without cleanup. List PRs in all states using the exact filters repository \`REMOTE_REPO\`, head \`BRANCH\`, and base branch \`BASE_BRANCH\`, and require every match to have those exact repository, head, and base identities. Require exactly one match. No match, multiple matches, or lookup failure must append \`blocked: direct-PR PR identity reconciliation failed; checkpoint retained\` and stop. If the sole match is closed or merged, append \`blocked: direct-PR PR is {url} ({state}); checkpoint retained\` and stop without creating a replacement. Require the sole open PR head OID to equal \`POST_HEAD\`; otherwise append \`blocked: direct-PR PR head moved; checkpoint retained\` and stop. After exact PR reconciliation, append exactly one nonterminal status line \`PR ready: {url} checkpoint=\$LEASE_CHECKPOINT task=\$TASK_ID workflow=\$WORKFLOW post_head=\$POST_HEAD\` and stop, retaining both exact state files. This existing actionable status must wake Firstmate immediately through the shared status classifier. Do not run \`fm-pr-check\`, remove checkpoint state, or emit \`done\`. Firstmate owns the single \`bin/fm-pr-check.sh "\$TASK_ID" "{url}"\` call and its metadata and poll writes. Only after Firstmate confirms that guarded check passed for the same canonical PR URL and retained checkpoint identity may the worker start a fresh step 10 invocation, repeat every endpoint, live-default, head, and PR validation, remove the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\`, and complete reconciliation.
