...
3. On recovery, detect active rebase metadata before remote-movement cleanup. An active rebase is valid only with \`PHASE=rebase-in-progress\` and metadata recording \`refs/heads/\$BRANCH\` as the original branch, \`PRE_HEAD\` as the original head, and \`DEFAULT_OID\` as the exact onto target; Git's expected detached \`HEAD\` is allowed only in that validated state, otherwise append \`blocked: active direct-PR rebase metadata mismatch\` and stop. Run \`git ls-remote --exit-code origin "\$FEATURE_REF"\` and classify its status: exit 0 supplies the remote OID, exit 2 means no matching ref, and any other status must append \`blocked: direct-PR lease recovery lookup failed\` and stop. For this bound post-conflict workflow, the remote matches only when exit 0 supplies \`EXPECTED\`; exit 2 is confirmed remote deletion, and any different OID from exit 0 is confirmed remote movement. If the remote moved during that validated active rebase, retain \`LEASE_CHECKPOINT\` and its bound state, append \`blocked: remote feature moved during active direct-PR rebase; checkpoint retained\`, and stop without cleanup or restart. If the remote moved with no active rebase, remove the validated checkpoint and its task-specific temporary artifacts, then restart safe validation from step 1. If the remote still matches, resume only the validated active rebase. With no active rebase, require attached \`CURRENT_BRANCH == BRANCH\`. When \`HEAD == PRE_HEAD\`, either atomically persist \`PRE_HEAD\` as \`POST_HEAD\` with \`PHASE=ready-to-push\` for a proven no-op where \`DEFAULT_OID\` is already an ancestor of \`HEAD\`, or safely restart \`git rebase "\$DEFAULT_OID"\` and perform the same ready-state rewrite after it completes. When \`HEAD\` differs, accept completed rebase recovery only when the latest metadata-bound matching rebase finish is the newest branch reflog entry, that finish result OID equals current \`HEAD\`, no later branch movement exists, the recorded branch equals \`BRANCH\`, the matching rebase transition's source OID equals \`PRE_HEAD\`, the matching rebase start names \`DEFAULT_OID\`, and \`DEFAULT_OID\` is an ancestor of \`HEAD\`. If the source OID differs from \`PRE_HEAD\`, the finish result differs from \`HEAD\`, or any later branch movement exists, append \`blocked: completed direct-PR rebase transition mismatch; refusing recovery\` and stop. Then atomically persist that exact \`HEAD\` as \`POST_HEAD\` with \`PHASE=ready-to-push\`. If that recovery rewrite fails, remove only the validated task-specific temporary artifact, append \`blocked: direct-PR recovered ready checkpoint write failed\`, and stop without pushing. For \`PHASE=ready-to-push\`, require \`WORKFLOW=post-conflict\`, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD\` to equal the nonempty \`POST_HEAD\`, then enter step 9 and execute its complete post-conflict publication workflow: remote classification, bounded identical retry, lease-rejection cleanup and restart, checkpoint cleanup, open-PR continuation, reconciliation completion, and the following single terminal completion or blocked status. Never perform only a bare push. Any other detached state or head, phase, workflow, branch, repository, task, ref, or onto mismatch must append \`blocked: direct-PR lease checkpoint state mismatch; refusing recovery\` and stop. Do not rerun pre-rebase ancestry validation against rewritten \`HEAD\`.
4. Run \`git fetch origin "+refs/heads/\$DEFAULT:refs/remotes/origin/\$DEFAULT"\`.
