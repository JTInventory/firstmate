...
1. Resolve \`DEFAULT\` from \`refs/remotes/origin/HEAD\`, falling back only to an existing local \`main\` or \`master\`. Set \`TASK_ID=$ID\`, \`FEATURE_REF=refs/heads/fm/$ID\`, \`BRANCH=fm/$ID\`, \`REPO_ID=\$(git rev-parse --path-format=absolute --git-common-dir)\`, \`LEASE_CHECKPOINT=$LEASE_CHECKPOINT_FILE\`, and \`LEASE_CHECKPOINT_TMP=\$LEASE_CHECKPOINT.tmp.\$\$\`. Derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\`; if \`HEAD\` is detached or \`CURRENT_BRANCH != BRANCH\`, append \`blocked: direct-PR requires attached branch fm/$ID\` and stop. This Firstmate-owned task state is outside the project worktree and Git index.
2. Reject a pre-existing checkpoint or temporary target that is a symlink, non-regular file, unexpectedly owned, or malformed by appending \`blocked: direct-PR lease checkpoint invalid; refusing recovery\` and stop. A valid checkpoint must contain exactly \`repo=\`, \`task=\`, \`feature_ref=\`, \`branch=\`, \`expected=\`, \`default_oid=\`, \`pre_head=\`, \`post_head=\`, and \`phase=\`. Require its repository, task, feature ref, branch, and the freshly derived attached \`CURRENT_BRANCH\` to equal \`REPO_ID\`, \`TASK_ID\`, \`FEATURE_REF\`, \`BRANCH\`, and \`BRANCH\`; otherwise append \`blocked: direct-PR lease checkpoint identity mismatch; refusing recovery\` and stop.
3. On recovery, query the remote feature ref; a lookup failure must append \`blocked: direct-PR lease recovery lookup failed\` and stop. If the remote moved from the checkpoint's \`expected=\`, remove the validated checkpoint and its task-specific temporary artifacts, then restart safe validation from step 1. If it still matches: for \`phase=rebase-in-progress\`, resume an active rebase only when its recorded original head equals \`pre_head=\`; if no rebase is active and \`HEAD\` differs from \`pre_head=\`, accept the completed rebase boundary only when \`ORIG_HEAD\` equals \`pre_head=\`, the latest branch reflog entry is the matching successful rebase finish, and \`default_oid=\` is an ancestor of \`HEAD\`, then atomically persist that exact \`HEAD\` as \`post_head=\` with \`phase=ready-to-push\`. If that recovery rewrite fails, remove only the validated task-specific temporary artifact, append \`blocked: direct-PR recovered ready checkpoint write failed\`, and stop without pushing. For \`phase=ready-to-push\`, require current \`HEAD\` to equal the nonempty \`post_head=\` and resume only the same explicit lease push. Before either recovery action, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again and require it to equal \`branch=\`; detached \`HEAD\` or any head, phase, branch, repository, task, or ref mismatch must append \`blocked: direct-PR lease checkpoint state mismatch; refusing recovery\` and stop. Do not rerun pre-rebase ancestry validation against rewritten \`HEAD\`.
