...
7. Set \`DEFAULT_OID=\$(git rev-parse refs/remotes/origin/\$DEFAULT)\` and \`PRE_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed before rebase\` and stop. Atomically write all thirteen bound fields, including \`remote_repo=\$REMOTE_REPO\` and \`base_ref=\$BASE_REF\`, through \`LEASE_CHECKPOINT_TMP\` with mode 0600, \`workflow=initial-publication\`, \`default_oid=\$DEFAULT_OID\`, \`post_head=\` empty, \`phase=rebase-in-progress\`, and \`attempt=0\`, then rename it to \`LEASE_CHECKPOINT\`. A write or rename failure must remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, append \`blocked: direct-PR lease checkpoint write failed\`, and stop.
8. Rebase onto the immutable \`DEFAULT_OID\` with \`git rebase "\$DEFAULT_OID"\` and resolve ordinary conflicts. Set \`POST_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed after rebase\` and stop. Atomically rewrite all thirteen bound fields through \`LEASE_CHECKPOINT_TMP\`, preserving \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, and \`pre_head=\$PRE_HEAD\`, while setting \`post_head=\$POST_HEAD\`, \`phase=ready-to-push\`, and \`attempt=0\`. If this second write or rename fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, append \`blocked: direct-PR ready checkpoint write failed; recover from rebase-in-progress state\`, and stop without pushing.
9. Before publication, require \`WORKFLOW=initial-publication\`, \`PHASE=ready-to-push\`, \`ATTEMPT\` equal to 0, 1, or 2, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`; otherwise append \`blocked: direct-PR publication state changed; refusing push\` and stop. The checkpoint attempt is the total authorized push count across every invocation. Immediately before every remote classification or push, repeat the step 1 canonical remote/base revalidation and retain the checkpoint on any blocker. Every attempt or exhaustion transition must atomically preserve \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, \`pre_head=\$PRE_HEAD\`, and \`post_head=\$POST_HEAD\`, change only the defined \`phase\` and \`attempt\`, write all thirteen validated fields with mode 0600 through stable \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. On any such write or rename failure, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, retain the primary checkpoint, append \`blocked: direct-PR publication checkpoint transition failed\`, and stop without pushing. Classify \`git ls-remote --exit-code origin "\$FEATURE_REF"\` as exit 0 with its OID, exit 2 with no ref, or any other status as \`blocked: remote feature retry lookup failed\` and stop. Exit 0 at \`POST_HEAD\` proves publication already succeeded; exit 0 at \`EXPECTED\`, or exit 2 only when \`EXPECTED\` is empty, means unchanged; every other exit 0 or 2 means movement. On movement, remove the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\`, then restart safe validation. For \`ATTEMPT=0\`, atomically rewrite the validated checkpoint with \`attempt=1\` and, after the rename succeeds, set \`ATTEMPT=1\` in memory before the first \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`; a transition failure uses that cleanup and blocker without pushing. If that push fails or recovery starts with \`ATTEMPT=1\`, classify the remote first: published proceeds to the published-state transition, movement restarts, and unchanged atomically rewrites \`attempt=2\` and, after the rename succeeds, sets \`ATTEMPT=2\` in memory before the second and final identical push. If that transition fails, use that cleanup and blocker without pushing. If the second push fails or recovery starts with \`ATTEMPT=2\`, classify the remote first: published proceeds to the published-state transition, movement restarts, and unchanged atomically rewrites \`phase=push-exhausted\` with \`attempt=2\`, retains the checkpoint, appends \`blocked: direct-PR publication retry exhausted; checkpoint retained\`, and stops without another push. Recovery from \`PHASE=push-exhausted\` performs no remote lookup or automatic push and retains that blocker. After either push succeeds or remote classification proves \`POST_HEAD\` is published, atomically preserve \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, \`pre_head=\$PRE_HEAD\`, \`post_head=\$POST_HEAD\`, and \`attempt=\$ATTEMPT\`, set \`phase=published-awaiting-pr\`, write all thirteen fields with mode 0600 through stable \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. If that write or rename fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, retain the durable prior checkpoint, append \`blocked: direct-PR published checkpoint transition failed; prior checkpoint retained\`, and stop before any PR action. Then enter step 10.
10. PR reconciliation is a separate \`PHASE=published-awaiting-pr\` path. Require \`WORKFLOW=initial-publication\`, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`. Immediately repeat the step 1 canonical remote/base revalidation, then revalidate \`git ls-remote --exit-code origin "\$FEATURE_REF"\`: exit 0 must return exactly \`POST_HEAD\`; exit 2 must append \`blocked: published direct-PR feature ref deleted; checkpoint retained\`; any other status must append \`blocked: published direct-PR feature lookup failed; checkpoint retained\`; and a different OID must append \`blocked: published direct-PR feature moved; checkpoint retained\`. Stop on every blocker without cleanup or PR creation. List PRs in all states using the exact filters repository \`REMOTE_REPO\`, head \`BRANCH\`, and base ref \`BASE_REF\`, and require every match to have those exact repository, head, and base identities. Multiple matches or lookup failure must append \`blocked: direct-PR PR identity reconciliation failed; checkpoint retained\` and stop. If the sole match is closed or merged, append \`blocked: direct-PR PR is {url} ({state}); checkpoint retained\` and stop without creating a replacement. If the sole match is open, require its head OID to equal \`POST_HEAD\`; otherwise append \`blocked: direct-PR PR head moved; checkpoint retained\` and stop. With no match, open exactly one PR with \`gh-axi\`, then re-list all states and require exactly one open match whose head OID equals \`POST_HEAD\`; any create or confirmation failure appends \`blocked: direct-PR publication succeeded but PR reconciliation failed; checkpoint retained\` and stops. Only after one open PR is confirmed at \`POST_HEAD\` may the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\` be removed. Never create a replacement for a closed or merged PR.
If firstmate later tells you that parallel work made the open PR conflict, you still own reconciliation:
1. Resolve \`DEFAULT\` from \`refs/remotes/origin/HEAD\`, falling back only to an existing local \`main\` or \`master\`, and set \`CURRENT_BASE_REF=refs/heads/\$DEFAULT\`. Resolve exactly one canonical push repository identity as \`CURRENT_REMOTE_REPO\` from \`git remote get-url --push --all origin\`; accept only one unambiguous GitHub SSH or HTTPS destination normalized to \`owner/repo\` without \`.git\`, and require \`git ls-remote --exit-code origin "\$CURRENT_BASE_REF"\` to return exactly one base OID. Lookup failure, base deletion, multiple push destinations, an unsupported URL, or ambiguity must append \`blocked: direct-PR remote or base identity lookup failed; checkpoint retained\` and stop. For a fresh workflow set \`REMOTE_REPO=\$CURRENT_REMOTE_REPO\` and \`BASE_REF=\$CURRENT_BASE_REF\`. Before every recovery, push, or PR lookup, repeat these live lookups and require the live canonical repository and base ref to equal the bound \`REMOTE_REPO\` and \`BASE_REF\`; retargeting, default-branch change, lookup failure, deletion, or ambiguity must append \`blocked: direct-PR remote or base identity changed; checkpoint retained\` and stop without cleanup. Set \`TASK_ID=$ID\`, \`FEATURE_REF=refs/heads/fm/$ID\`, \`BRANCH=fm/$ID\`, \`WORKFLOW=post-conflict\`, \`REPO_ID=\$(git rev-parse --path-format=absolute --git-common-dir)\`, \`LEASE_CHECKPOINT=$LEASE_CHECKPOINT_FILE\`, and \`LEASE_CHECKPOINT_TMP=\$LEASE_CHECKPOINT.tmp\`. Derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` when attached. An attached branch must equal \`BRANCH\`; a detached \`HEAD\` is allowed only when \`LEASE_CHECKPOINT\` exists and step 3 proves an active rebase for the bound branch and base. Otherwise append \`blocked: direct-PR requires attached branch fm/$ID\` and stop. This Firstmate-owned task state is outside the project worktree and Git index.
2. Validate the primary checkpoint before handling the stable task-specific temporary target. Reject a checkpoint that is a symlink, non-regular file, unexpectedly owned, not mode 0600 according to \`fm_pr_file_mode\`, or malformed by appending \`blocked: direct-PR lease checkpoint invalid; refusing recovery\` and stop. If \`LEASE_CHECKPOINT_TMP\` exists, reject it when it is a symlink, non-regular file, or unexpectedly owned; otherwise remove that exact same-task stale temporary file without parsing it, preserving the primary checkpoint. A valid checkpoint must contain exactly \`repo=\`, \`remote_repo=\`, \`base_ref=\`, \`task=\`, \`feature_ref=\`, \`branch=\`, \`workflow=\`, \`expected=\`, \`default_oid=\`, \`pre_head=\`, \`post_head=\`, \`phase=\`, and \`attempt=\`. Parse it only as data: read exactly one line for each literal key, split each line only at its first \`=\`, and assign \`CHECKPOINT_REPO\`, \`CHECKPOINT_REMOTE_REPO\`, \`CHECKPOINT_BASE_REF\`, \`CHECKPOINT_TASK\`, \`CHECKPOINT_FEATURE_REF\`, \`CHECKPOINT_BRANCH\`, \`CHECKPOINT_WORKFLOW\`, \`CHECKPOINT_EXPECTED\`, \`CHECKPOINT_DEFAULT_OID\`, \`CHECKPOINT_PRE_HEAD\`, \`CHECKPOINT_POST_HEAD\`, \`CHECKPOINT_PHASE\`, and \`CHECKPOINT_ATTEMPT\`; reject duplicate, missing, unknown, or invalid fields. Never source or eval checkpoint bytes. Require the checkpoint identities to equal \`REPO_ID\`, \`TASK_ID\`, \`FEATURE_REF\`, and \`BRANCH\`, require \`CHECKPOINT_REMOTE_REPO == CURRENT_REMOTE_REPO\` and \`CHECKPOINT_BASE_REF == CURRENT_BASE_REF\`, and require \`CHECKPOINT_WORKFLOW=post-conflict\`; when \`HEAD\` is attached, also require freshly derived \`CURRENT_BRANCH == BRANCH\`. Otherwise append \`blocked: direct-PR lease checkpoint identity mismatch; refusing recovery\` and stop. After all validation, explicitly hydrate \`REPO_ID=\$CHECKPOINT_REPO\`, \`REMOTE_REPO=\$CHECKPOINT_REMOTE_REPO\`, \`BASE_REF=\$CHECKPOINT_BASE_REF\`, \`TASK_ID=\$CHECKPOINT_TASK\`, \`FEATURE_REF=\$CHECKPOINT_FEATURE_REF\`, \`BRANCH=\$CHECKPOINT_BRANCH\`, \`WORKFLOW=\$CHECKPOINT_WORKFLOW\`, \`EXPECTED=\$CHECKPOINT_EXPECTED\`, \`DEFAULT_OID=\$CHECKPOINT_DEFAULT_OID\`, \`PRE_HEAD=\$CHECKPOINT_PRE_HEAD\`, \`POST_HEAD=\$CHECKPOINT_POST_HEAD\`, \`PHASE=\$CHECKPOINT_PHASE\`, and \`ATTEMPT=\$CHECKPOINT_ATTEMPT\` before any recovery action. Enforce the exact phase tuple before action: \`rebase-in-progress\` requires \`ATTEMPT=0\` and empty \`POST_HEAD\`; \`ready-to-push\` requires nonempty \`POST_HEAD\` and \`ATTEMPT\` 0, 1, or 2; \`push-exhausted\` requires nonempty \`POST_HEAD\` and \`ATTEMPT=2\`; \`published-awaiting-pr\` requires nonempty \`POST_HEAD\` and \`ATTEMPT\` 0, 1, or 2. Any other tuple must append \`blocked: direct-PR lease checkpoint state mismatch; refusing recovery\` and stop. Fresh-shell rule: never rely on variables surviving a prior agent command call. Before every command invocation in steps 2-10, use that same invocation to repeat step 1 identity initialization and, when the checkpoint exists, this complete typed validation and hydration. Before the checkpoint exists, a fresh invocation in steps 4-7 must also replay every earlier state-producing fetch or lookup needed by that step. Rebase continuation, post-rebase checkpointing, publication, retry, cleanup, and open-PR continuation each require the prelude in their own invocation.
3. On recovery, branch on \`PHASE\` before inspecting \`HEAD\`, reflogs, or active-rebase metadata. If \`PHASE=published-awaiting-pr\`, require the bound workflow, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD\` to equal the nonempty \`POST_HEAD\`, then enter the separate step 10 PR-reconciliation path directly; never enter step 9 or perform another push. If \`PHASE=push-exhausted\`, require \`ATTEMPT=2\`, the bound workflow, attached branch, and exact \`POST_HEAD\`; retain the checkpoint, append \`blocked: direct-PR publication retry exhausted; checkpoint retained\`, and stop without any automatic push. If \`PHASE=ready-to-push\`, require \`ATTEMPT\` to be 0, 1, or 2 and require \`WORKFLOW=post-conflict\`, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD\` to equal the nonempty \`POST_HEAD\`, then enter step 9 directly and execute its complete post-conflict publication workflow: remote classification, bounded identical retry, lease-rejection cleanup and restart, transition to published-awaiting-pr, separate open-PR reconciliation, and reconciliation completion, and the following single terminal completion or blocked status. Never perform only a bare push. Only when \`PHASE=rebase-in-progress\` may recovery inspect active-rebase metadata, \`HEAD\`, or reflogs; any other phase must append \`blocked: direct-PR lease checkpoint state mismatch; refusing recovery\` and stop. In that rebase-in-progress branch, detect active rebase metadata before remote-movement cleanup. An active rebase is valid only when metadata records \`refs/heads/\$BRANCH\` as the original branch, \`PRE_HEAD\` as the original head, and \`DEFAULT_OID\` as the exact onto target; Git's expected detached \`HEAD\` is allowed only in that validated state, otherwise append \`blocked: active direct-PR rebase metadata mismatch\` and stop. Run \`git ls-remote --exit-code origin "\$FEATURE_REF"\` and classify its status: exit 0 supplies the remote OID, exit 2 means no matching ref, and any other status must append \`blocked: direct-PR lease recovery lookup failed\` and stop. For this bound post-conflict workflow, the remote matches only when exit 0 supplies \`EXPECTED\`; exit 2 is confirmed remote deletion, and any different OID from exit 0 is confirmed remote movement. If the remote moved during that validated active rebase, retain \`LEASE_CHECKPOINT\` and its bound state, append \`blocked: remote feature moved during active direct-PR rebase; checkpoint retained\`, and stop without cleanup or restart. If the remote moved with no active rebase, remove the validated checkpoint and its stable task-specific \`LEASE_CHECKPOINT_TMP\`, then restart safe validation from step 1. If the remote still matches, resume only the validated active rebase. With no active rebase, require attached \`CURRENT_BRANCH == BRANCH\`. When \`HEAD == PRE_HEAD\`, either use the complete thirteen-field atomic transition to preserve \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, and \`pre_head=\$PRE_HEAD\`, set \`post_head=\$PRE_HEAD\`, \`phase=ready-to-push\`, and \`attempt=0\`, write with mode 0600 through \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\` for a proven no-op where \`DEFAULT_OID\` is already an ancestor of \`HEAD\`; or safely restart \`git rebase "\$DEFAULT_OID"\` and perform that same complete ready-state transition after it completes. When \`HEAD\` differs, accept completed rebase recovery only when the latest metadata-bound matching rebase finish is the newest branch reflog entry, that finish result OID equals current \`HEAD\`, no later branch movement exists, the recorded branch equals \`BRANCH\`, the matching rebase transition's source OID equals \`PRE_HEAD\`, the matching rebase start names \`DEFAULT_OID\`, and \`DEFAULT_OID\` is an ancestor of \`HEAD\`. If the source OID differs from \`PRE_HEAD\`, the finish result differs from \`HEAD\`, or any later branch movement exists, append \`blocked: completed direct-PR rebase transition mismatch; refusing recovery\` and stop. Then use the complete thirteen-field atomic transition to preserve \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, and \`pre_head=\$PRE_HEAD\`, set \`post_head\` to that exact \`HEAD\`, \`phase=ready-to-push\`, and \`attempt=0\`, write with mode 0600 through \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. If that recovery rewrite fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, append \`blocked: direct-PR recovered ready checkpoint write failed\`, and stop without pushing. After any successful ready-state rewrite, start a fresh invocation with the required prelude and enter step 9. Any other detached state or head, workflow, branch, repository, task, ref, or onto mismatch must append \`blocked: direct-PR lease checkpoint state mismatch; refusing recovery\` and stop. Do not rerun pre-rebase ancestry validation against rewritten \`HEAD\`.
4. Run \`git fetch origin "+\$BASE_REF:refs/remotes/origin/\$DEFAULT"\`.
...
7. Set \`DEFAULT_OID=\$(git rev-parse refs/remotes/origin/\$DEFAULT)\` and \`PRE_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed before rebase\` and stop. Atomically write all thirteen bound fields, including \`remote_repo=\$REMOTE_REPO\` and \`base_ref=\$BASE_REF\`, through \`LEASE_CHECKPOINT_TMP\` with mode 0600, \`workflow=post-conflict\`, \`default_oid=\$DEFAULT_OID\`, \`post_head=\` empty, \`phase=rebase-in-progress\`, and \`attempt=0\`, then rename it to \`LEASE_CHECKPOINT\`. A write or rename failure must remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, append \`blocked: direct-PR lease checkpoint write failed\`, and stop.
8. Rebase onto the immutable \`DEFAULT_OID\` with \`git rebase "\$DEFAULT_OID"\` and resolve ordinary conflicts. Set \`POST_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed after rebase\` and stop. Atomically rewrite all thirteen bound fields through \`LEASE_CHECKPOINT_TMP\`, preserving \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, and \`pre_head=\$PRE_HEAD\`, while setting \`post_head=\$POST_HEAD\`, \`phase=ready-to-push\`, and \`attempt=0\`. If this second write or rename fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, append \`blocked: direct-PR ready checkpoint write failed; recover from rebase-in-progress state\`, and stop without pushing.
9. Before publication, require \`WORKFLOW=post-conflict\`, \`PHASE=ready-to-push\`, \`ATTEMPT\` equal to 0, 1, or 2, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`; otherwise append \`blocked: direct-PR publication state changed; refusing push\` and stop. The checkpoint attempt is the total authorized push count across every invocation. Immediately before every remote classification or push, repeat the step 1 canonical remote/base revalidation and retain the checkpoint on any blocker. Every attempt or exhaustion transition must atomically preserve \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, \`pre_head=\$PRE_HEAD\`, and \`post_head=\$POST_HEAD\`, change only the defined \`phase\` and \`attempt\`, write all thirteen validated fields with mode 0600 through stable \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. On any such write or rename failure, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, retain the primary checkpoint, append \`blocked: direct-PR publication checkpoint transition failed\`, and stop without pushing. Classify \`git ls-remote --exit-code origin "\$FEATURE_REF"\` as exit 0 with its OID, exit 2 with no ref, or any other status as \`blocked: remote feature retry lookup failed\` and stop. Exit 0 at \`POST_HEAD\` proves publication already succeeded; exit 0 at \`EXPECTED\` means unchanged; a different OID or exit 2 means movement. On movement, remove the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\`, then restart safe validation. For \`ATTEMPT=0\`, atomically rewrite the validated checkpoint with \`attempt=1\` and, after the rename succeeds, set \`ATTEMPT=1\` in memory before the first \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`; a transition failure uses that cleanup and blocker without pushing. If that push fails or recovery starts with \`ATTEMPT=1\`, classify the remote first: published proceeds to the published-state transition, movement restarts, and unchanged atomically rewrites \`attempt=2\` and, after the rename succeeds, sets \`ATTEMPT=2\` in memory before the second and final identical push. If that transition fails, use that cleanup and blocker without pushing. If the second push fails or recovery starts with \`ATTEMPT=2\`, classify the remote first: published proceeds to the published-state transition, movement restarts, and unchanged atomically rewrites \`phase=push-exhausted\` with \`attempt=2\`, retains the checkpoint, appends \`blocked: direct-PR publication retry exhausted; checkpoint retained\`, and stops without another push. Recovery from \`PHASE=push-exhausted\` performs no remote lookup or automatic push and retains that blocker. After either push succeeds or remote classification proves \`POST_HEAD\` is published, atomically preserve \`repo=\$REPO_ID\`, \`remote_repo=\$REMOTE_REPO\`, \`base_ref=\$BASE_REF\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`workflow=\$WORKFLOW\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, \`pre_head=\$PRE_HEAD\`, \`post_head=\$POST_HEAD\`, and \`attempt=\$ATTEMPT\`, set \`phase=published-awaiting-pr\`, write all thirteen fields with mode 0600 through stable \`LEASE_CHECKPOINT_TMP\`, and rename it to \`LEASE_CHECKPOINT\`. If that write or rename fails, remove only the validated stable \`LEASE_CHECKPOINT_TMP\`, retain the durable prior checkpoint, append \`blocked: direct-PR published checkpoint transition failed; prior checkpoint retained\`, and stop before any PR action. Then enter step 10.
10. PR reconciliation is a separate \`PHASE=published-awaiting-pr\` path. Require \`WORKFLOW=post-conflict\`, attached \`CURRENT_BRANCH == BRANCH\`, and current \`HEAD == POST_HEAD\`. Immediately repeat the step 1 canonical remote/base revalidation, then revalidate \`git ls-remote --exit-code origin "\$FEATURE_REF"\`: exit 0 must return exactly \`POST_HEAD\`; exit 2 must append \`blocked: published direct-PR feature ref deleted; checkpoint retained\`; any other status must append \`blocked: published direct-PR feature lookup failed; checkpoint retained\`; and a different OID must append \`blocked: published direct-PR feature moved; checkpoint retained\`. Stop on every blocker without cleanup. List PRs in all states using the exact filters repository \`REMOTE_REPO\`, head \`BRANCH\`, and base ref \`BASE_REF\`, and require every match to have those exact repository, head, and base identities. Require exactly one match. No match, multiple matches, or lookup failure must append \`blocked: direct-PR PR identity reconciliation failed; checkpoint retained\` and stop. If the sole match is closed or merged, append \`blocked: direct-PR PR is {url} ({state}); checkpoint retained\` and stop without creating a replacement. Require the sole open PR head OID to equal \`POST_HEAD\`; otherwise append \`blocked: direct-PR PR head moved; checkpoint retained\` and stop. Rerun \`fm-pr-check\` and require exit zero. On nonzero append \`blocked: direct-PR post-conflict PR check failed; checkpoint retained\`, retain both exact state files, and stop. Only after exit zero remove the validated checkpoint and stable task-specific \`LEASE_CHECKPOINT_TMP\`; only then is reconciliation complete.
