...
9. Before pushing, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again and require \`CURRENT_BRANCH == BRANCH\` and \`\$(git rev-parse HEAD) == POST_HEAD\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR publication state changed; refusing push\` and stop. Push with \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`. If the push fails and the remote still matches the checkpoint, retry that identical \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`; if the retry lookup fails, append \`blocked: remote feature retry lookup failed\` and stop. After a lease rejection or confirmed remote movement, remove the validated checkpoint and its task-specific temporary artifacts and restart safe validation. After the push succeeds, remove the validated checkpoint and its task-specific temporary artifacts, then open the PR with \`gh-axi\`.
...
7. Set \`DEFAULT_OID=\$(git rev-parse refs/remotes/origin/\$DEFAULT)\` and \`PRE_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed before rebase\` and stop. Atomically write all nine bound fields through \`LEASE_CHECKPOINT_TMP\` with mode 0600, \`default_oid=\$DEFAULT_OID\`, \`post_head=\` empty, and \`phase=rebase-in-progress\`, then rename it to \`LEASE_CHECKPOINT\`. A write or rename failure must remove only the validated task-specific temporary artifact, append \`blocked: direct-PR lease checkpoint write failed\`, and stop.
8. Rebase onto the immutable \`DEFAULT_OID\` with \`git rebase "\$DEFAULT_OID"\` and resolve ordinary conflicts. Set \`POST_HEAD=\$(git rev-parse HEAD)\`, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again, and require \`CURRENT_BRANCH == BRANCH\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR branch changed after rebase\` and stop. Atomically rewrite all nine bound fields through \`LEASE_CHECKPOINT_TMP\`, preserving \`repo=\$REPO_ID\`, \`task=\$TASK_ID\`, \`feature_ref=\$FEATURE_REF\`, \`branch=\$BRANCH\`, \`expected=\$EXPECTED\`, \`default_oid=\$DEFAULT_OID\`, and \`pre_head=\$PRE_HEAD\`, while setting \`post_head=\$POST_HEAD\` and \`phase=ready-to-push\`. If this second write or rename fails, remove only the validated task-specific temporary artifact, append \`blocked: direct-PR ready checkpoint write failed; recover from rebase-in-progress state\`, and stop without pushing.
9. Before pushing, derive \`CURRENT_BRANCH=\$(git symbolic-ref --quiet --short HEAD)\` again and require \`CURRENT_BRANCH == BRANCH\` and \`\$(git rev-parse HEAD) == POST_HEAD\`; detached \`HEAD\` or mismatch must append \`blocked: direct-PR publication state changed; refusing push\` and stop. Update the open PR with \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`. If the push fails and the remote still matches the checkpoint, retry that identical \`git push --force-with-lease="\$FEATURE_REF:\$EXPECTED" origin "\$POST_HEAD:\$FEATURE_REF"\`; if the retry lookup fails, append \`blocked: remote feature retry lookup failed\` and stop. After a lease rejection or confirmed remote movement, remove the validated checkpoint and its task-specific temporary artifacts and restart safe validation. After the push succeeds, remove the validated checkpoint and its task-specific temporary artifacts; only then is reconciliation complete.
