...
# outcome=passed with the pr and ci steps skipped - mirrors the incident shape
# where delivery never ran, so the pipeline observed no merge.
...
# outcome=passed with delivery completed - the ci monitor ran to completion.
...
test_terminal_passed_delivery_skipped() {
  ...
  assert_contains "$out" "state: unknown" "passed with skipped delivery -> unknown"
