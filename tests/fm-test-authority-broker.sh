...
if [ "${FM_TEST_AUTHORITY_PROVISION_PRIMARY:-0}" = 1 ]; then
  ...
  broker_command='unset FM_TEST_AUTHORITY_PROVISION_PRIMARY; FM_TEST_AUTHORITY_BROKER_PID=$$ FM_TEST_AUTHORITY_OWNER_PID=$$ FM_TEST_AUTHORITY_HARNESS_PID=$PPID FM_SESSION_AUTHORITY_PID=$$ FM_SESSION_AUTHORITY_BROKER_PID=$$ FM_SESSION_AUTHORITY_BROKER_SCRIPT="$1" FM_SESSION_AUTHORITY_HOME="$4" FM_SESSION_AUTHORITY_CHECKOUT="$3"; export FM_TEST_AUTHORITY_BROKER_PID FM_TEST_AUTHORITY_OWNER_PID FM_TEST_AUTHORITY_HARNESS_PID FM_SESSION_AUTHORITY_PID FM_SESSION_AUTHORITY_BROKER_PID FM_SESSION_AUTHORITY_BROKER_SCRIPT FM_SESSION_AUTHORITY_HOME FM_SESSION_AUTHORITY_CHECKOUT; ROOT=$3; export ROOT; . "$3/bin/fm-session-lock-lib.sh"; exec 9<&19; FM_SESSION_AUTHORITY_FD=9 FM_SESSION_AUTHORITY_DURABLE_FD=18 FM_TEST_AUTHORITY_FD=9 FM_TEST_DURABLE_AUTHORITY_FD=18; export FM_SESSION_AUTHORITY_FD FM_SESSION_AUTHORITY_DURABLE_FD FM_TEST_AUTHORITY_FD FM_TEST_DURABLE_AUTHORITY_FD; fm_worker_test_primary_identity_bind "$3" "$4" "$5" || exit 1; exec 19<&-; exec /usr/bin/bash "$1" /usr/bin/bash "$2"'
