...
write_task_meta() {
  ...
  if [ ! -d "$dir/wt/.git" ]; then
    git -C "$dir/wt" init -q
  fi
  git -C "$dir/wt" checkout -B "fm/$id" -q
...
test_invalid_entrypoints_have_zero_side_effects() {
  ...
  for value in "${INVALID_URLS[@]}"; do
    ...
    grep -q '^error:' "$dir/stderr" || fail "merge invalid URL diagnostic was not fixed"
    after=$(state_snapshot "$dir/home/state")
...
SH
    ...
    git -C "$dir/missing-worktree" init -q
    git -C "$dir/missing-worktree" checkout -B "fm/$id" -q
...
test_concurrent_watcher_sees_only_complete_publication() {
  ...
  while [ "$n" -le 3 ]; do
    ...
    write_task_meta "$dir"
    # This case isolates poll publication. Do not let the fork's stricter
    # backend-capture wake win the race before the delayed poll is published.
    sed -i '/^window=/d' "$dir/home/state/task-a.meta"
    cat > "$dir/fakebin/cp" <<SH
#!/usr/bin/env bash
'$REAL_CP' "\$@" || exit 1
sleep 0.3
SH
    ...
    grep -q '^check: .*: merged$' "$dir/watch.out" \
      || fail "concurrent watcher never saw complete poll: out=$(cat "$dir/watch.out") err=$(cat "$dir/watch.err")"
