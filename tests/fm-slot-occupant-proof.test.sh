...
fm_agent_proc_cwd() {
  # The old global census reaches an unrelated live process and becomes
  # unprovable. Exact endpoint proof must never consult that process.
...
generation_file="$TMP_ROOT/generation-reads"
: > "$generation_file"
fm_session_process_start() {
  local count
  count=$(cat "$generation_file")
  count=$((count + 1))
  printf '%s\n' "$count" > "$generation_file"
  if [ "$count" -eq 1 ]; then
    printf '%s' proc:first
  else
    printf '%s' proc:second
  fi
}
fm_session_process_identity() {
  printf '%s' exe:/usr/bin/sleep
}
fm_agent_proc_cwd() {
  [ "$1" = "$ENDPOINT_PID" ] || return 1
  printf '%s' "$WORKTREE"
