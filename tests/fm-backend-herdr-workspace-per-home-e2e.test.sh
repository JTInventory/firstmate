#!/usr/bin/env bash
# tests/fm-backend-herdr-workspace-per-home-e2e.test.sh - opt-in ISOLATED
...
#   - live workspace inventory seeing only each home's own tabs
set -u

if [ "${FM_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set FM_HERDR_E2E=1 to run the real workspace-per-home Herdr lab e2e"
  exit 0
fi
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" cm1 "$PROJ1" "sh -c 'echo primary-crew-ok'" --backend herdr --scout \
  >"$CM1_OUT" 2>"$CM1_ERR"
rc=$?
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$SM_HOME" FM_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/fm-spawn.sh" cm2 "$PROJ2" "sh -c 'echo sm-crew-ok'" --backend herdr --scout \
  >"$CM2_OUT" 2>"$CM2_ERR"
rc=$?
...
# --- 4. live workspace contents stay isolated per home ---------------------
# JT #73 exposes tab inventory through list_task_ids rather than the owner's
# later list_live helper. Read each real workspace directly and assert the same
...
# The disposable scratch projects are intentionally detached Treehouse leases.
# JT's ship teardown refuses detached worktrees by design, while scout teardown
# is the supported scratch-worktree path. Reports make this a real guarded
