...
# (the same restart identity mechanism docs/herdr-backend.md already documents:
...
if [ "${FM_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set FM_HERDR_E2E=1 to run the real respawn-idempotency Herdr lab e2e"
  exit 0
fi
