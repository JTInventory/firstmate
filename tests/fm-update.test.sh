...
SH
  ...
  if (
    ...
    unset FM_SESSION_AUTHORITY_BROKER_IDENTITY
    ...
    fail "pre-port primary migrated legacy metadata without trusted authority"
  fi
  if (
    PATH="$ownership_fakebin:$PATH"
    FM_FAKE_TMUX_LOG="$w/ownership-fake/tmux.log"
  ...
  if (
    ...
    fail "declaration-less pre-port task bypassed coordinated restart"
  fi
  [ "$(grep -c '^tmux_pane_id=' "$ownership_meta")" -eq 0 ] \
    && [ "$(grep -c '^endpoint_generation=' "$ownership_meta")" -eq 0 ] \
    || fail "declaration-less pre-port refusal mutated metadata"
  crash_meta="$w/home/state/crash.meta"
  mkdir -p "$w/crash"
  printf 'crash\n' > "$w/crash/.fm-secondmate-home"
  {
    printf 'window=firstmate:fm-crash\n'
    printf 'kind=secondmate\n'
    printf 'harness=codex\n'
    printf 'home=%s/crash\n' "$w"
  } > "$crash_meta"
  ...
  if (
    ...
    fm_backend_tmux_migration_commit_meta() { return 1; }
  ...
  if (
    ...
    fm_agent_environ() {
      printf 'FM_AGENT_TASK=committed\nFM_AGENT_OWNER_HOME=%s\nFM_AGENT_ROLE=secondmate\n' \
        "$w/committed"
    }
    endpoint_checks=0
