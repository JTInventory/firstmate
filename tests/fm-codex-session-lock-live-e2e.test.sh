#!/usr/bin/env bash
# Opt-in real Codex /quit -> SessionEnd regression. No provider request is made.
set -u
...
printf '/quit\r' \
  | timeout 60 env -u CODEX_THREAD_ID FM_HOME="$HOME_DIR" FM_ROOT_OVERRIDE="$ROOT" \
...
grep -F '/quit' "$TRANSCRIPT" >/dev/null || fail "Codex transcript did not contain /quit"
