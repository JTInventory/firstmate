...
#   - output section ordering: safety preamble leads, READ-ONCE and live fleet
#     state precede bulky curated context
...
#   - per-line status-tail cap and truncation marker
#   - startup backlog composition: done rows dropped, in-flight/held/blocked
#     rows retained, ready queued rows bounded with a disclosed remainder
...
test_lock_write_failure_read_only_path() {
  ...
  if [ "$(id -u)" -eq 0 ]; then
    pass "session-lock publication failure test skipped as root (root bypasses directory write bits)"
    return 0
  fi
...
EOF
  ...
  [ "$wake_line" -lt "$read_once_line" ] || fail "WAKE QUEUE did not precede READ-ONCE CONTRACT"
  [ "$read_once_line" -lt "$fleet_line" ] || fail "READ-ONCE CONTRACT did not precede FLEET STATE"
  ...
  [ "$inventory_line" -lt "$context_line" ] || fail "live task inventory was buried behind curated context"
  assert_contains "$out" "Captain memory that may be truncated away safely." \
    "ordering fixture did not print the curated memory"
  ...
  pass "digest sections put READ-ONCE and live fleet state before curated context"
}
...
EOF
  ...
  assert_contains "$out" "Do NOT re-read any of them after reading this digest" \
    "READ-ONCE CONTRACT lost its core instruction"
  assert_contains "$out" "this digest was truncated before a section appeared" \
    "READ-ONCE CONTRACT does not explain how truncation changes the read boundary"
  assert_contains "$out" "section above governs what may still be read" \
    "closing reminder does not point back at READ-ONCE CONTRACT"

  contract_count=$(printf '%s\n' "$out" | grep -c 'Do NOT re-read any of them')
  [ "$contract_count" -eq 1 ] || fail "READ-ONCE CONTRACT was repeated $contract_count times"

  pass "READ-ONCE CONTRACT is emitted once before the sources it governs"
}
...
test_status_tail_line_cap() {
  local rec root home fakebin out lede tail_section longest capped
...
EOF
  ...
  assert_contains "$out" "$lede" "line cap discarded the decision lede"
  assert_contains "$out" " [truncated]" "over-long status line was not marked"
  assert_contains "$out" "working: short line kept whole" "short status line was changed"
  assert_contains "$out" "each capped at 220 characters" "status-tail header did not disclose the cap"
  assert_contains "$out" "$home/state/task-cap.status" "full status path was lost"
  ...
  [ "$longest" -le 220 ] || fail "status tail emitted a $longest-character line past the cap"
  capped=$(printf '%s\n' "$tail_section" | grep -c ' \[truncated\]$')
  [ "$capped" -eq 1 ] || fail "expected one truncated status line, got $capped"

  pass "status-tail lines are capped at 220 characters with a recoverable marker"
}
...
EOF
  ...
  assert_contains "$out" "ready-3,queued,ship,firstmate,Ready item 3" \
    "tasks-axi compact listing omitted a ready queued row within the bound"
  ...
  assert_not_contains "$out" "DONE-ROW-LINE" "tasks-axi compact digest listed a done row"
  ...
  assert_contains "$out" "ready_public_followups: 0 delivery-ready obligations" \
    "composed tasks-axi output dropped the public-followup signal"
  assert_not_contains "$out" "help[1]:" "composed tasks-axi output repeated per-group help"
  assert_grep "--state in_flight --fields blocked_by,hold_kind,hold_reason" "$log" \
    "session start did not ask tasks-axi for in-flight rows"
  assert_grep "--state held --fields blocked_by,hold_kind,hold_reason" "$log" \
    "session start did not ask tasks-axi for held rows"
  assert_grep "--state queued --blocked --fields blocked_by,hold_kind,hold_reason" "$log" \
    "session start did not ask tasks-axi for blocked queued rows"
  assert_grep "ready --file $home/data/backlog.md" "$log" \
    "session start did not ask tasks-axi for ready queued rows"

  pass "compatible tasks-axi backlog rendering preserves recovery rows and drops done rows"
}
...
EOF
  ...
  assert_contains "$out" "ready-3,queued,ship,firstmate,Ready item 3" \
    "queued bound dropped a row inside its limit"
  assert_not_contains "$out" "ready-4,queued" "queued bound did not cap ready rows"
  assert_contains "$out" "(shown 3 of 7 ready queued item(s))" \
    "queued bound did not report shown and total counts"
  assert_contains "$out" "(4 more queued - tasks-axi ready --file $home/data/backlog.md)" \
    "queued bound did not disclose the exact remainder"
  assert_contains "$out" "held-queued,queued,ship,firstmate,Held queued work,none,captain,captain choice pending" \
    "queued bound swallowed a held row"
  assert_contains "$out" 'blocked-followup,queued,scout,firstmate,Follow compact startup,compact-startup,"-","-"' \
    "queued bound swallowed a blocked row"
  assert_contains "$out" "compact-startup,in_flight,ship,firstmate,Compact startup digest,none,captain,captain choice pending" \
    "queued bound swallowed an in-flight row"

  pass "queued bound affects only ready work and discloses the omitted remainder"
}
...
EOF
  ...
  assert_not_contains "$out" "DONE-ROW-LINE" "manual compact rendering listed a done row"
  assert_not_contains "$out" "## Done" "manual compact rendering printed the done heading"
...
EOF
  ...
  assert_not_contains "$out" "DONE-ROW-LINE" "unavailable tasks-axi fallback listed a done row"
