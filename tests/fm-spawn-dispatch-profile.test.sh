...
run_spawn() {
  ...
  FM_ROOT_OVERRIDE='' FM_HOME="$home" \
    ...
    FM_SPAWN_NO_GUARD=1 FM_RUN_STEP_ID="$1" FM_FAKE_PANE_PATH="$wt" \
    FM_FAKE_TMUX_STATE="$home/tmux-window-name" TMUX="fake,1,0" \
...
test_grok_omits_invalid_xhigh_reasoning_effort() {
  ...
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" \
    "$id" "$PROJ_DIR" --model grok-4 --effort xhigh)
  status=$?
  expect_code 0 "$status" "grok xhigh should be omitted rather than passed"
  assert_meta_profile "$HOME_DIR/state/$id.meta" grok grok-4 xhigh
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "grok --always-approve --model 'grok-4' \"\$(cat " \
    "grok launch did not preserve the model when xhigh was omitted"
  assert_not_contains "$launch" "--reasoning-effort" "grok launch must omit xhigh reasoning effort"
  assert_not_contains "$launch" "--effort" "grok launch must not use the --effort alias"
