...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` is the canonical ShellCheck entrypoint; CI enforces it.
...
bin/fm-lint.sh                             # canonical lint entrypoint; CI enforces this
for test_script in tests/*.test.sh; do "$test_script"; done   # complete local behavior-test pass; CI runs its focused subset
...
tests/fm-backend-herdr-presentation-e2e.test.sh # focused Herdr adapter launch/teardown proof (see docs/worker-isolation.md); requires jq and an installed Herdr protocol >=16, and preflights the host without mutating its default session
