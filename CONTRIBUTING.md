...
`bin/fm-no-mistakes-pr-target-guard.sh` checks local `origin`, all `origin` push URLs, the no-mistakes remote target, and `no-mistakes status` before the test suite runs, so stale gate state cannot open or update a PR on `kunchenguid/firstmate`.
