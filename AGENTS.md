...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, the read-only worker-isolation sweep, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The worker-isolation sweep is read-only and reports actionable `ISOLATION:` findings when live process evidence proves a task is outside its recorded worktree or belongs to another home, and when required identity, scope, or process-cwd evidence is unavailable or ambiguous and isolation therefore remains fail-closed `unproven`.
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap alive/dead read of each task's recorded backend endpoint.
   The endpoint target is resolved from recorded metadata through the passive backend reader; this digest never starts a provider or upgrades a read into live-binding or mutation authority.
...
For ship and scout tasks, the script creates the selected backend endpoint (in your current tmux session or a dedicated `firstmate` session for tmux, or in the selected Herdr session), acquires the task-bound durable lease described in [the worktree contract](docs/architecture.md#worktrees-not-branches-in-your-checkout), waits for the leased worktree subshell, asserts the resolved worktree is a genuine worktree of the target project (matching physical git common dir and target-repo HEAD, refusing publication and retaining ownership evidence if exact endpoint retirement cannot be proven, to prevent wrong-repo launches and the worktree tangle of section 8), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
With `--force`, teardown is the explicit discard path for child windows, child work, state, route, lease, and home, but it never waives pooled-slot ownership: a contested slot is retained and a contested secondmate home is refused (see [worker isolation](docs/worker-isolation.md) and [secondmate provisioning](.agents/skills/secondmate-provisioning/SKILL.md)). Never use it unless the captain explicitly said to discard the work.
...
Two further guards prevent the tangle upstream: `fm-spawn` applies the durable-leased target-worktree contract in [the worktree contract](docs/architecture.md#worktrees-not-branches-in-your-checkout), and every ship brief's first instruction has the crewmate verify it is in its own worktree before branching (section 11).
