...
- **direct-PR** - no pipeline. The crewmate pushes and opens the PR itself (its brief says so) and reports `done: PR <url>`. Before opening the PR, resolve the default branch from `refs/remotes/origin/HEAD` with the repository's `main`/`master` fallback, explicitly fetch both the default ref and any existing `refs/heads/fm/<id>` feature ref, snapshot the feature ref's expected OID, rebase onto the fetched default ref, and resolve ordinary conflicts. If parallel work later makes the open PR conflict, the same crewmate owns direct-PR reconciliation and repeats that fetch/snapshot/rebase before updating the published branch with `git push --force-with-lease=refs/heads/fm/<id>:<expected-oid> origin HEAD:refs/heads/fm/<id>`; an empty expected OID is allowed only after confirming the remote feature ref does not exist. Report `done: PR <url>` once after opening or reconciling the PR. Skip the Validate step and go straight to PR ready; after any reconciliation, rerun `fm-pr-check` before relaying the PR. Teardown uses the normal landed-work check.
