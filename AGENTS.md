...
A direct-PR post-conflict handoff instead reports the existing actionable `PR ready: <url>` status with its retained checkpoint identity. Run `bin/fm-pr-check.sh <id> <PR url>` once at that handoff, resume the worker after it passes, and relay the already-armed PR when the worker emits its single terminal `done`; do not publish PR metadata or arm the merge poll again.
