...
- The direct-PR checkpoint also binds a durable publication attempt count. Each authorized push atomically increments it before the push, two attempts exhaust the workflow across interruptions, and `push-exhausted` recovery retains the blocker without another automatic push. Every phase and attempt transition preserves all eleven validated fields through one stable task-specific temporary checkpoint target, and transition failure removes only that temporary target while retaining the durable primary checkpoint. The direct-PR worktree exception covers exactly those two task-bound state files.
