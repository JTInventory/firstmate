...
backups/             root-local preservation files; not a canonical tracked surface
  ...
  <id>.pr-presentation   private immutable receipt written only by fm-pr-present.sh; binds the full GitHub PR URL to the exact head shown to the captain and is never refreshed by polling
...
The wrapper requires the protected presentation receipt, verifies the current forge head still equals the presented head, defaults to squash, and uses GitHub's atomic merge API with that exact SHA. Missing, malformed, changed, or unverifiable heads fail closed and invalidate stale approval; `yolo` never bypasses this boundary.
