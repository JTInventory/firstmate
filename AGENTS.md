...
A direct-PR post-conflict handoff instead reports the existing actionable `PR ready: <url>` status with its retained checkpoint identity. Revalidate the exact repository, head, base, and head OID against the checkpoint before running `bin/fm-pr-check.sh <id> <PR url>` once. After it passes, atomically preserve the full checkpoint identity and record its canonical URL and exact publication head in a mode-0600 `pr-check-confirmed` receipt. Resume the worker only from that exact receipt; after interruption, reuse it instead of publishing PR metadata or arming the merge poll again.
