...
The local behavior-test command is `bin/fm-run-behavior-tests.sh`: it keeps the PR-target guard and full test glob, requires `tmux`, and isolates each test's `TMPDIR` and `GOTMPDIR`; CI continues to run its own full suite loop.
