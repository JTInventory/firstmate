...
The wrapper requires the protected presentation receipt, verifies its unique nonce plus the approved and current URL/head/base tuple, defaults to squash, and uses GitHub's atomic merge API with that exact SHA. It preserves immediate merge methods, literal commit subject/body, matched-head, and safely encoded remote branch deletion options. Deferred auto-merge, disabling auto-merge, admin bypass, and author-email overrides are deliberately unsupported because they cannot use this immediate expected-head merge path. Missing, malformed, changed, or unverifiable presentation identity fails closed and invalidates stale approval; `yolo` never bypasses this boundary.
