...
It is config-only: it uses the same live secondmate discovery, per-home inheritance lock, `propagate_inheritable_config` helper, and `CONFIG_REREAD` delivery path as bootstrap, prints a per-home/per-item summary, and does not fast-forward tracked files.
