...
backups/             root-local preservation files; not a canonical tracked surface
  ...
  <id>.pr-presentation   private immutable receipt written only by fm-pr-present.sh; binds the full GitHub PR URL to the exact head shown to the captain and is never refreshed by polling; presentation and merge serialize replacement and consumption
...
Immediately before telling the captain, run `bin/fm-pr-present.sh <id> <full GitHub PR URL>`. It revalidates the PR and freezes the exact presented head in a separate protected receipt; ordinary checks and polls never rewrite it. Tell the captain only after that command succeeds. If the PR head changes, present it again and obtain fresh approval.
...
If the captain says "merge it", run `FM_CAPTAIN_APPROVED_MERGE=1 FM_CAPTAIN_APPROVED_PR_HEAD=<sha printed by fm-pr-present.sh> bin/fm-pr-merge.sh <id> <full GitHub PR URL>`; those explicit environment markers bind the approval to that presentation and prevent direct `gh-axi pr merge` use. Local-only work keeps `bin/fm-merge-local.sh`.

The wrapper requires the protected presentation receipt, verifies the approved and current forge heads still equal the presented head, defaults to squash, and uses GitHub's atomic merge API with that exact SHA. It preserves immediate merge methods, commit subject/body, matched-head, and remote branch deletion options. Deferred auto-merge, disabling auto-merge, admin bypass, and author-email overrides are deliberately unsupported because they cannot use this immediate expected-head merge path. Missing, malformed, changed, or unverifiable heads fail closed and invalidate stale approval; `yolo` never bypasses this boundary.
