...
- **direct-PR** - no pipeline. The crewmate pushes and opens the PR itself (its brief says so) and reports `done: PR <url>`. Before opening the PR, resolve the default branch from `refs/remotes/origin/HEAD` with the repository's `main`/`master` fallback, explicitly fetch `+refs/heads/<default>:refs/remotes/origin/<default>`, rebase onto that fetched ref, and resolve ordinary conflicts. If parallel work later makes the open PR conflict, the same crewmate owns direct-PR reconciliation and repeats that explicit fetch/rebase before updating the published branch with `git push --force-with-lease origin fm/<id>` and reporting `done: PR <url>` again. Skip the Validate step and go straight to PR ready; after any reconciliation, rerun `fm-pr-check` before relaying the PR. Teardown uses the normal landed-work check.
