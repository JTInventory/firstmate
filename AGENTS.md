...
- **direct-PR** - no pipeline. The crewmate pushes and opens the PR itself (its brief says so) and reports `done: PR <url>`. Before opening the PR, fetch `origin` and rebase onto its fetched authoritative default-branch ref. If parallel work later makes the open PR conflict, the same crewmate owns direct-PR reconciliation: fetch `origin`, rebase onto that fetched ref, resolve ordinary conflicts, update the published branch with `git push --force-with-lease origin fm/<id>`, and report `done: PR <url>` again. Skip the Validate step and go straight to PR ready; after any reconciliation, rerun `fm-pr-check` before relaying the PR. Teardown uses the normal landed-work check.
